...
class UserCommands(commands.Cog):
    ...
    async def note_auto(
        ...
        return [
            app_commands.Choice(
                name=user_data["notes"][note]["topic"],
                value=user_data["notes"][note]["topic"],
            )
    ...
    notepad = app_commands.Group(
        name="notepad",
        description="Notepad commands",
    ...
    async def create_notepad(
        self, interaction: Interaction, topic: app_commands.Range[str, 1, 100]
    ):
        user_data = await self.notepad.find(interaction.user.id)
        if not user_data:
            user_data = {
        ...
        modal.content = discord.ui.TextInput(
            label="Content",
            required=True,
        ...
        await modal.interaction.response.send_message(
            f"Succesfully created notepad with topic {topic}", ephemeral=True
        )
    ...
    async def view_notepad(self, interaction: Interaction):
        ...
        if not user_data:
            await interaction.response.send_message(
                "You do not have a notepad", ephemeral=True
            )
            return
        pages = []
        ...
        if len(pages) == 0:
            await interaction.response.send_message(
                "You do not have any notes", ephemeral=True
            )
            return
        await Paginator(interaction=interaction, pages=pages).start(
            embeded=True, timeout=60, quick_navigation=False, hidden=True
        )
    ...
    async def delete_notepad(self, interaction: Interaction, topic: str):
        ...
        await self.notepad.update(interaction.user.id, user_data)
        await interaction.response.send_message(
            f"Succesfully deleted notepad with topic {topic}", ephemeral=True
        )
    ...
    async def edit_notepad(self, interaction: Interaction, topic: str):
        ...
        view = General_Modal(
            title=f"Editing notepad with topic {topic}", interaction=interaction
        )
        view.content = discord.ui.TextInput(
            label="New content",
        ...
        await view.interaction.response.send_message(
            f"Successfully edited notepad with topic {topic}", ephemeral=True
        )

    @app_commands.command(
        name="dank_calculator", description="Calculate the dankest meme"
    )
    @app_commands.describe(
        item="The item you want to calculate",
        quantity="The quantity of the item you want to calculate",
    )
    ...
    async def dank_calculator(self, interaction: Interaction, item: str, quantity: int):
        ...
        await interaction.response.send_message(
            f"The total value of `{quantity}x` **{item}** is ⏣ `{total:,}`",
            ephemeral=True,
        )
