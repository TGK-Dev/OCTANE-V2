...
class Perks(commands.Cog, name="perk", description="manage your custom perks"):
    ...
    async def interaction_check(self, interaction: discord.Interaction):
        data = await self.backend.bans.find(
    ...
    async def on_check_profile_roles(self, data: Custom_Roles):
        ...
        if not crole:
            return
        config = await self.backend.get_data(
            self.backend.types.config, guild.id, self.bot.user.id
        )
        if not config:
            return
        ...
        if not isinstance(user, discord.Member):
            ...
            if log_channel:
                await log_channel.send(
                    f"**User**: {data['user_id']} has left the server and his custom role `{crole.name}` will been deleted"
                )
        ...
        for key, item in config["profiles"]["roles"].items():
            ...
            if role in user.roles:
                if item["duration"] == "permanent":
                    total_duraction = "permanent"
                else:
                    total_duraction += item["duration"]
                if total_share_limit < 30:
                    total_share_limit += item["share_limit"]
                elif total_share_limit >= 30:
                    total_share_limit = 30
        ...
        if total_duraction == 0 and data["freeze"]["delete"] is not True:
            ...
            if channel:
                await channel.send(
                    f"**User**: {user.mention} is going to lose his custom role `{crole.name}`",
                    allowed_mentions=discord.AllowedMentions.none(),
                )
            try:
                await user.send(
                    embed=discord.Embed(
                        description=f"Your custom role `{role.name}` has been deleted because you have no active custom roles",
    ...
    async def on_check_profile_channels(self, data: Custom_Channel):
        guild: discord.Guild = self.bot.get_guild(data["guild_id"])
        config = await self.backend.get_data(
            self.backend.types.config, guild.id, data["user_id"]
        )
        if not guild:
            return
        ...
        if not isinstance(user, discord.Member):
            ...
            if log_channel:
                await log_channel.send(
                    f"**User**: {data['user_id']} has left the server and his custom channel `{channel.name}` has been deleted"
                )
        ...
        if total_duraction == 0 and data["freeze"]["delete"] is not True:
            ...
            if channel:
                await channel.send(
                    f"**User**: {user.mention} is going to lose his custom channel `{channel.name}`"
                )
            try:
                await user.send(
                    embed=discord.Embed(
                        description=f"Your custom channel `{channel.name}` has been deleted because you have no active custom channels",
                        color=self.bot.default_color,
                    )
                )
            except Exception:
    ...
    async def check_channel_activity(self):
        ...
        for data in config:
            ...
            channels: List[
                Custom_Channel
            ] = await self.backend.channel.find_many_by_custom({"guild_id": guild.id})
            for channel in channels:
                await asyncio.sleep(1)
                if (
                    channel["activity"] is None
                    or channel["activity"]["last_message"] is None
                ):
                    chl = guild.get_channel(channel["channel_id"])
                    channel["activity"] = {
                        "messages": 0,
                        "last_message": datetime.datetime.utcnow(),
                        "previous_cat": chl.category.id,
                    }
                ...
                if datetime.datetime.utcnow() - channel["activity"][
                    "last_message"
                    ...
                    log_channel = guild.get_channel(1190668526361518120)
                    await log_channel.send(
                        f"**User**: {channel['user_id']} is going to lose his custom channel {chanel.mention} because it has been inactive for more than 7 days",
                        allowed_mentions=discord.AllowedMentions.none(),
                    )
                    try:
                        await channel_owner.send(
                            embed=discord.Embed(
                                description=f"Your custom channel {chanel.name} in {guild.name} has been deleted because it has been inactive for more than 7 days",
                                color=self.bot.default_color,
                            )
                        )
                    except Exception:
        ...
        for key, item in config["profiles"]["reacts"].items():
            ...
            if role in user.roles:
                if item["duration"] == "permanent":
                    ...
                    total_duraction += item["duration"]
                if total_reaction_limit < 10:
                    total_reaction_limit += item["share_limit"]
                elif total_reaction_limit >= 10:
                    total_reaction_limit = 10

        if total_duraction == 0:
            chal = self.bot.get_channel(1190668526361518120)
            await chal.send(
                f"**User**: {user.mention} is going to lose his custom react {data['emojis']}"
            )
            try:
                await user.send(
                    embed=discord.Embed(
                        description="Your custom react has been deleted because you have no active custom reacts",
                        color=self.bot.default_color,
                    )
                )
            except Exception:
    ...
    async def highlight_remove_auto(
        self, interaction: Interaction, current: str
    ) -> List[app_commands.Choice[str]]:
        user_data = await self.backend.get_data(
            self.backend.types.highlights, interaction.guild.id, interaction.user.id
        )
        if user_data is None:
            return [app_commands.Choice(value="none", name="none")]
        else:
            triggers_list = [
            ...
            if len(user_data["triggers"]) == 0:
                triggers_list = [app_commands.Choice(value="none", name="none")]
            return triggers_list[:24]

    privrole = app_commands.Group(
        name="privrole", description="Manage your custom roles"
    )
    privchannel = app_commands.Group(
        name="privchannel", description="Manage your custom channels"
    )
    privreact = app_commands.Group(
        name="privreact", description="Manage your custom reacts"
    )
    highlight = app_commands.Group(
        name="highlight", description="Manage your custom highlights"
    )
    privemoji = app_commands.Group(
        name="privemoji", description="Manage your custom emojis"
    )
    perks = app_commands.Group(name="perks", description="Manage your custom perks")
    admin = app_commands.Group(
        name="admin", description="Manage your custom perks", parent=perks
    )
    ...
    async def _prole(self, interaction: Interaction):
        ...
        if not config:
            return await interaction.edit_original_message(
                content="Server has no custom perks"
            )
        embed = discord.Embed(color=interaction.client.default_color, description="")
        embed.set_author(
            name=f"{interaction.user}'s Private Roles",
            icon_url=interaction.user.display_avatar.url
            if interaction.user.display_avatar
            else interaction.user.default_avatar,
        )
        user_data = await self.backend.get_data(
            self.backend.types.roles, interaction.guild.id, interaction.user.id
        )
        if not user_data:
            return await interaction.edit_original_response(
                content="You have no custom role use /privrole claim to create one"
            )
        ...
        embed.add_field(
            name=" ",
            value=f"**Owner**: <@{user_data['user_id']}>\n**Role**: {role.mention}",
        )
        embed.add_field(
            name=" ",
            value=f"**Friend Limit**: {user_data['share_limit']}"
            + "\n**Friend List:**\n"
            + "\n".join(
                [
                    f"`{user_data['friend_list'].index(friend) + 1}.` <@{friend}>"
        ...
        embed.add_field(
            name=" ",
            value=f"**Duration**: {humanfriendly.format_timespan(user_data['duration']) if user_data['duration'] != 'permanent' else 'Permanent'}",
        )
    ...
    @app_commands.describe(
        name="name of your custom role",
        color="color of your custom role like #2b2d31",
        icon="role icon of your custom role",
    )
    async def _prole_claim(
        self,
        ...
        for key, item in config["profiles"]["roles"].items():
            ...
            if role in interaction.user.roles:
                if item["duration"] == "permanent":
                    ...
                    total_duraction += item["duration"]
                if total_share_limit < 10:
                    total_share_limit += item["share_limit"]
                elif total_share_limit >= 10:
                    total_share_limit = 10

        if total_duraction == 0:
            return await interaction.response.send_message(
                "You have no active custom roles", ephemeral=True
            )

        await interaction.response.send_message(
            embed=discord.Embed(
                description="Creating your custom role...",
        ...
        if "AmariMod" in name:
            return await interaction.edit_original_message(
                content="Role name cannot contain AmariMod", embed=None
            )
        if len(name) > 100:
            return await interaction.edit_original_message(
                content="Role name cannot be longer than 30 characters", embed=None
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(icon.url) as resp:
                    if resp.status != 200:
                        await interaction.edit_original_response(
                            embed=discord.Embed(
                                description="Failed to download the icon",
                                color=interaction.client.default_color,
                            )
                        )
                        return
        ...
        if "#" not in color:
            return await interaction.edit_original_response(
                embed=discord.Embed(
                    description="Invalid color make sure to add `#` before the hex code",
                    color=interaction.client.default_color,
                )
            )
        ...
        user_data = await self.backend.create(
            ...
            duration=total_duraction,
            share_limit=total_share_limit,
        )
        ...
        await interaction.edit_original_response(
            embed=discord.Embed(
                description=f"Role {role.mention} created successfully",
                color=interaction.client.default_color,
            )
        )
    ...
    async def _prole_edit(
        ...
        if name is None and color is None and icon is None:
            return await interaction.response.send_message(
                "You need to provide at least one argument", ephemeral=True
            )
        user_data = await self.backend.get_data(
            self.backend.types.roles, interaction.guild.id, interaction.user.id
        )
        ...
        if not user_data:
            return await interaction.response.send_message(
                "You have no custom role use /privrole claim to create one",
                ephemeral=True,
            )
        if name:
            if "AmariMod" in name:
        ...
        if not role:
            return await interaction.response.send_message(
                content="Role not found", embed=None
            )
        await interaction.response.send_message(
            embed=discord.Embed(
                description="Updating your custom role...",
                color=interaction.client.default_color,
            )
        )
        keywords = {}
        ...
        if icon:
            keywords["display_icon"] = icon
        await role.edit(**keywords)

        await interaction.edit_original_response(
            embed=discord.Embed(
                description=f"Role {role.mention} updated successfully",
                color=interaction.client.default_color,
            )
        )
    ...
    async def _prole_friend(self, interaction: Interaction):
        ...
        role = interaction.guild.get_role(user_data["role_id"])
        embed = discord.Embed(
            title=f"{interaction.user}'s Custom Role Friends",
            color=interaction.client.default_color if not role.color else role.color,
            description="",
        )
    ...
    async def _pchannel(self, interaction: Interaction):
        embed = discord.Embed(color=interaction.client.default_color, description="")
        embed.set_author(
            name=f"{interaction.user.name}'s Private channel",
            icon_url=interaction.user.display_avatar.url
            if interaction.user.display_avatar
            else interaction.user.default_avatar,
        )
        user_data = await self.backend.get_data(
            self.backend.types.channels, interaction.guild.id, interaction.user.id
        )
        if not user_data:
            return await interaction.response.send_message(
                "You have no custom channel use /privchannel claim to create one",
                ephemeral=True,
            )
    ...
    async def _pchannel_claim(self, interaction: Interaction, name: str):
        user_data = await self.backend.channel.find(
            {"user_id": interaction.user.id, "guild_id": interaction.guild.id}
        )
        config = await self.backend.get_data(
            self.backend.types.config, interaction.guild.id, interaction.user.id
        ...
        if total_duraction == 0:
            return await interaction.response.send_message(
                "You have no active custom channels", ephemeral=True
            )

        await interaction.response.send_message(
            embed=discord.Embed(
                description="Creating your custom channel...",
                color=interaction.client.default_color,
            )
        )
        category = None
        for cat in config["custom_category"]["cat_list"]:
            ...
            if len(cat.channels) < 10:
                ...
                break
        if not category:
            last_cat = interaction.guild.get_channel(
                config["custom_category"]["last_cat"]
            )
            category = await interaction.guild.create_category_channel(
                ...
                overwrites=last_cat.overwrites,
                reason="Custom Category",
            )
        ...
        channel = await interaction.guild.create_text_channel(
            name=name,
            category=category,
            topic=f"Private channel of {interaction.user.name}",
            overwrites=overwrites,
        )

        user_data = await self.backend.create(
            self.backend.types.channels,
            interaction.user.id,
            interaction.guild.id,
            duration=total_duraction,
            share_limit=total_share_limit,
        ...
        await channel.send(
            f"Welcome to your private channel {interaction.user.mention}"
        )
    ...
    async def _pchannel_edit(self, interaction: Interaction, name: str):
        ...
        if not channel:
            return await interaction.edit_original_response(
                content="Channel not found", embed=None
            )
        await interaction.response.send_message(
            embed=discord.Embed(
                description="Updating your custom channel...",
                color=interaction.client.default_color,
            )
        )
        ...
        await interaction.edit_original_response(
            embed=discord.Embed(
                description=f"Channel {channel.mention} updated successfully",
                color=interaction.client.default_color,
            )
        )
    ...
    async def _preact(self, interaction: Interaction):
        user_data = await self.backend.get_data(
            self.backend.types.reacts, interaction.guild.id, interaction.user.id
        )
        if not user_data:
            return await interaction.response.send_message(
                "You have no custom react use /privreact claim to create one",
                ephemeral=True,
            )

        embed = discord.Embed(color=interaction.client.default_color, description="")
        embed.set_author(
            name=f"{interaction.user}'s Private Reacts",
            icon_url=interaction.user.display_avatar.url
            if interaction.user.display_avatar
            else interaction.user.default_avatar,
        )
        ...
        embed.add_field(
            name="Emojis", value=",".join([f"{emoji}" for emoji in user_data["emojis"]])
        )
        await interaction.response.send_message(embed=embed, content=None)

    @privchannel.command(
        name="friend", description="Manage your custom channel friend list"
    )
    ...
    async def _pchannel_friend(self, interaction: Interaction):
        ...
        embed = discord.Embed(
            title=f"{interaction.user}'s Custom Channel Friends",
            color=interaction.client.default_color,
            description="",
        )
        embed.description += f"**Friends Limit:** {user_data['share_limit']}\n"
    ...
    async def _preact_claim(self, interaction: Interaction, emoji: str):
        ...
        if user_data:
            return await interaction.response.send_message(
                "You already have a custom react use /privreact edit to edit it",
                ephemeral=True,
            )

        await interaction.response.send_message(
            embed=discord.Embed(
                description="Creating your custom react...",
                color=interaction.client.default_color,
            )
        )
        config = await self.backend.get_data(
        ...
        if total_duraction == 0:
            return await interaction.edit_original_response(
                embed=discord.Embed(
                    description="You have no active custom reacts",
                    color=interaction.client.default_color,
                )
            )
        else:
            await interaction.edit_original_response(
                embed=discord.Embed(
                    description="Verifying emoji...",
                    color=interaction.client.default_color,
                )
            )

        user_data = await self.backend.create(
            self.backend.types.reacts,
            interaction.user.id,
            interaction.guild.id,
            duration=total_duraction,
            share_limit=total_emojis,
        )
        ...
        try:
            ...
            return await interaction.edit_original_response(
                embed=discord.Embed(
                    description="Invalid emoji", color=interaction.client.default_color
                )
            )
        ...
        await self.backend.update_cache(
            self.backend.types.reacts, interaction.guild, user_data
        )
        await interaction.edit_original_response(
            embed=discord.Embed(
                description=f"React {emoji} created successfully",
                color=interaction.client.default_color,
            )
        )
    ...
    async def _preact_edit(
        self, interaction: Interaction, action: Literal["add", "remove"], emoji: str
    ):
        user_data = await self.backend.get_data(
            self.backend.types.reacts, interaction.guild.id, interaction.user.id
        )
        ...
        if action == "add":
            ...
            await interaction.edit_original_response(
                embed=discord.Embed(
                    description=f"Emoji {emoji} added successfully",
                    color=interaction.client.default_color,
                )
            )
            ...
            await interaction.edit_original_response(
                embed=discord.Embed(
                    description=f"Emoji {emoji} removed successfully",
                    color=interaction.client.default_color,
                )
            )
    ...
    async def highlight_trigger(self, interaction: Interaction, trigger: str):
        ...
        if not user_data:
            ...
            if total_duraction == 0:
                return await interaction.response.send_message(
                    "You have no active custom highlights", ephemeral=True
                )
            user_data = await self.backend.create(
                self.backend.types.highlights,
                interaction.user.id,
                interaction.guild.id,
                duration=total_duraction,
                share_limit=total_trigger,
            )

        if len(user_data["triggers"]) >= user_data["tigger_limit"]:
            return await interaction.response.send_message(
                "You have reached the maximum amount of triggers", ephemeral=True
            )
        if trigger.lower() in user_data["triggers"]:
            return await interaction.response.send_message(
                "You already have that trigger", ephemeral=True
            )

        user_data["triggers"].append(trigger.lower())
        await self.backend.update(self.backend.types.highlights, user_data)
        await self.backend.update_cache(
            self.backend.types.highlights, interaction.guild, user_data
        )

        await interaction.response.send_message(
            f"Your trigger `{trigger}` has been added", ephemeral=True
        )

    @highlight.command(
        name="tremove", description="remove a trigger from your highlight perk"
    )
    ...
    async def highlight_trigger_remove(self, interaction: Interaction, trigger: str):
        ...
        if not user_data:
            return await interaction.response.send_message(
                "You don't have any highlight perks", ephemeral=True
            )
        if trigger.lower() not in user_data["triggers"]:
            return await interaction.response.send_message(
                "You don't have that trigger", ephemeral=True
            )
        ...
        await interaction.response.send_message(
            f"Your trigger `{trigger}` has been removed", ephemeral=True
        )

    @highlight.command(
        name="ignore", description="ignore a user from your highlight perk"
    )
    @app_commands.checks.cooldown(1, 5.0, key=lambda i: (i.guild.id, i.user.id))
    async def highlight_ignore_role(self, interaction: Interaction):
        ...
        embed.description += (
            "Users:"
            + f"{', '.join([f'<@{i}>' for i in user_data['ignore_users']]) if user_data['ignore_users'] else '`None`'}"
        )
        embed.description += (
            "\nChannels:"
            + f"{', '.join([f'<#{i}>' for i in user_data['ignore_channel']]) if user_data['ignore_channel'] else '`None`'}"
        )
    ...
    async def _highlight_show(self, interaction: Interaction):
        ...
        if not user_data:
            return await interaction.response.send_message(
                "You have no custom highlight use /highlight tadd to create one",
                ephemeral=True,
            )

        embed = discord.Embed(color=interaction.client.default_color, description="")
        formated_args = await get_formated_embed(
            ["Triggers Limit", "Triggers", "Ignore Users", "Ignore Channels"]
        )
    ...
    async def _pemoji(self, interaction: Interaction):
        user_data = await self.backend.get_data(
            self.backend.types.emojis, interaction.guild.id, interaction.user.id
        )
        if not user_data:
            return await interaction.response.send_message(
                "You have no custom emoji use /privemoji claim to create one",
                ephemeral=True,
            )
    ...
    @app_commands.describe(
        emoji="emoji of your custom emoji", name="name of your custom emoji"
    )
    async def _pemoji_claim(
        self,
        interaction: Interaction,
        emoji: discord.Attachment,
        name: app_commands.Range[str, 1, 20],
    ):
        await interaction.response.send_message(
            embed=discord.Embed(
        ...
        user_profile_info = await self.backend.calulate_profile(
            self.backend.types.emojis, interaction.guild, interaction.user
        )
        if user_profile_info["duration"] == 0:
            return await interaction.edit_original_response(
                content=None,
                embed=discord.Embed(
                    description="You have no active custom emojis",
                    color=interaction.client.default_color,
                ),
            )

        already_requested = await self.backend.emoji_request.find(
        ...
        if already_requested:
            await interaction.edit_original_response(
                content=None,
                embed=discord.Embed(
                    description="You already have a pending request for a custom emoji please wait for the admin to respond",
                    color=interaction.client.default_color,
                ),
            )
            return
        ...
        if not config:
            return await interaction.edit_original_response(
                content=None,
                embed=discord.Embed(
                    description="Server has no custom perks",
                    color=interaction.client.default_color,
                ),
            )
        ...
        if len(all_emojis) >= config["emojis"]["max"]:
            return await interaction.edit_original_response(
                content=None,
                embed=discord.Embed(
                    description="Server has reached the max emoji limit",
                    color=interaction.client.default_color,
                ),
            )

        emoji_reqeust_channel = interaction.guild.get_channel(
            config["emojis"]["request_channel"]
        )
        if not emoji_reqeust_channel:
            return await interaction.edit_original_response(
                content=None,
                embed=discord.Embed(
                    description="Emoji request channel not found",
                    color=interaction.client.default_color,
                ),
            )
        ...
        req_message = await emoji_reqeust_channel.send(
            content=None, embed=embed, file=file, view=Emoji_Request()
        )
    ...
    @app_commands.describe(
        member="The member you want to manage", perk="The perk you want to manage"
    )
    async def _premove(
        ...
        if len(config["admin_roles"]) == 0:
            await interaction.response.send_message(
                "You need to setup admin roles first", ephemeral=True
            )
            return
        ...
        if (set(user_roles) & set(config["admin_roles"])) == set():
            await interaction.response.send_message(
                "You need to have admin roles to use this command", ephemeral=True
            )
            return

        if perk == self.backend.types.config:
            await interaction.response.send_message(
                "You can't remove config", ephemeral=True
            )
            return
        perk_data = await self.backend.get_data(perk, interaction.guild.id, member.id)
        if not perk_data:
            await interaction.response.send_message(
                "This user doesn't have this perk", ephemeral=True
            )
            return
    ...
    @app_commands.describe(
        ...
        channel="The channel you want to search for",
    )
    async def _psearch(
        self,
        interaction: Interaction,
        ...
        if role is None and channel is None:
            return await interaction.response.send_message(
                "You need to specify a role or a channel", ephemeral=True
            )
        embeds = []
        if role:
            role_data = await self.backend.roles.find(
                {"role_id": role.id, "guild_id": interaction.guild.id}
            )
            if not role_data:
                return await interaction.response.send_message(
            ...
            duration = (
                humanfriendly.format_timespan(role_data["duration"])
                if role_data["duration"] != "permanent"
                else "Permanent"
            )
            ...
            role_embed.description += (
                "**Friends:**"
                + ", ".join([f"<@{friend}>" for friend in role_data["friend_list"]])
                if len(role_data["friend_list"]) > 0
                else "None"
            )
            embeds.append(role_embed)
        if channel:
            channel_data = await self.backend.channel.find(
            ...
            duration = (
                humanfriendly.format_timespan(channel_data["duration"])
                if channel_data["duration"] != "permanent"
                else "Permanent"
            )
            channel_embed = discord.Embed(
                title="Custom Channel Info",
                color=interaction.client.default_color,
                description="",
            )
            ...
            channel_embed.description += (
                f"**Friend Limit:** {channel_data['share_limit']}\n"
            )
            channel_embed.description += (
                "**Friends:**"
                + ", ".join([f"<@{friend}>" for friend in channel_data["friend_list"]])
                if len(channel_data["friend_list"]) > 0
                else "None"
            )
            channel_embed.add_field(
                name="Activiy",
                value=f"**Rank: **{channel_data['activity']['rank']}\n**Messages:** {channel_data['activity']['messages']}\n",
            )
    ...
    @app_commands.describe(
        member="The member you want to block", reason="The reason of the block"
    )
    async def _ban(self, interaction: Interaction, member: discord.Member, reason: str):
        ...
        await interaction.response.send_message(
            f"{member.mention} has been blocked from using custom perks clearing all their perks this may take few seconds",
            ephemeral=True,
        )
        role_data = await self.backend.get_data(
            self.backend.types.roles, interaction.guild.id, member.id
        )
        channel_data = await self.backend.get_data(
            self.backend.types.channels, interaction.guild.id, member.id
        )
        react_data = await self.backend.get_data(
            self.backend.types.reacts, interaction.guild.id, member.id
        )
        highlight_data = await self.backend.get_data(
            self.backend.types.highlights, interaction.guild.id, member.id
        )
        ...
        if role_data:
            ...
            if role:
                await role.delete(
                    reason=f"Perk Owner blocked by {interaction.user.name}"
                )
        ...
        await interaction.followup.send(
            f"All existing perks of {member.mention} has been cleared successfully",
            ephemeral=True,
        )
    ...
    async def _unban(self, interaction: Interaction, member: discord.Member):
        ...
        if (set(user_roles) & set(config["admin_roles"])) == set():
            ...
            return
        ban_data = await self.backend.bans.find(
            {"user_id": member.id, "guild_id": interaction.guild.id}
        )
        if not ban_data:
            await interaction.response.send_message(
                "This user is not blocked", ephemeral=True
            )
            return
        await self.backend.bans.delete(ban_data)
        await interaction.response.send_message(
            f"{member.mention} has been unblocked from using custom perks",
            ephemeral=True,
        )

    @admin.command(
        name="freez", description="Freez a custom perk's share limit/deletion"
    )
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(
        member="The member you want to freez",
        perk="The perk you want to freez",
        type="which attribute you want to freez",
    ...
    async def _freez(
        ...
        perk: Literal["roles", "channel"],
        type: Literal["share", "delete"],
        value: bool,
    ):
        config = await self.backend.get_data(
            self.backend.types.config, interaction.guild.id, interaction.user.id
        ...
        match perk:
            case "roles":
                perk_data: Custom_Roles = await self.backend.get_data(
                    self.backend.types.roles, interaction.guild.id, member.id
                )
            case "channel":
                perk_data: Custom_Channel = await self.backend.get_data(
                    self.backend.types.channels, interaction.guild.id, member.id
                )
            case _:
        ...
        await interaction.response.send_message(
            f"Successfully updated {member.mention}'s {perk} {type} freeze to {value}",
            ephemeral=True,
        )
    ...
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(
        member="The member you want to delete the perk from",
    ...
    @admin.command(
        name="sync-top-cat", description="sync top category with all channels"
    )
    @app_commands.default_permissions(administrator=True)
    async def _sync_top_cat(self, interaction: Interaction):
        ...
        top_cat = interaction.guild.get_channel(
            config["top_channel_category"]["cat_id"]
        )
        if not top_cat:
            await interaction.response.send_message(
                "Top category not found", ephemeral=True
            )
            return

        await interaction.response.send_message(
            "Syncing all channels with the top category"
        )
        ...
        for chl in await self.backend.channel.find_many_by_custom(
            {"guild_id": interaction.guild.id}
        ):
            chl: Custom_Channel
            ...
            if (set(owner.roles) & set(top_roles)) == set():
                if channel.category != top_cat:
                    pass
                else:
                    await channel.edit(
                        category=interaction.guild.get_channel(
                            chl["activity"]["previous_cat"]
                        )
                    )
            else:
                if channel.category == top_cat:
        ...
        await interaction.edit_original_response(
            content="All channels has been synced with the top category"
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.author.id != custom_channel["user_id"]:
            if (
                ...
                and message._interaction.user.id != custom_channel["user_id"]
            ):
                return
        ...
        if custom_channel["activity"]["last_message"] is not None:
            if not datetime.datetime.utcnow() > custom_channel["activity"][
                "last_message"
    ...
    async def on_auto_highlight(self, message: discord.Message):
        if message.guild.id not in self.backend.cach["highlight"].keys():
            return
        ...
        for word in message_content:
            for user_data in guild_data:
                ...
                if word in user_data["triggers"]:
                    ...
                    if not perm.view_channel or not perm.view_channel:
                        continue
    ...
    async def on_highlight_found(self, message: discord.Message, user_data: dict):
        ...
        if (
            message.channel.id in user_data["ignore_channel"]
            or message.author.id in user_data["ignore_users"]
        ):
            return

        before_messages = [
            message
            async for message in message.channel.history(limit=10, before=message)
        ]

        embed = discord.Embed(
            title=f"Hightlight found in {message.guild.name}",
            color=self.bot.default_color,
            description="",
        )
        ...
        embed.add_field(
            ...
            value=f"[<t:{round(message.created_at.timestamp())}:T>] {message.author.display_name}: {message.content}",
            inline=False,
        )
        embed.set_footer(
            text=f"Triggered by {message.author.global_name}",
        ...
        await self.backend.update_cache(
            self.backend.types.highlights, message.guild, user_data
        )
        await self.backend.update(self.backend.types.highlights, user_data)
    ...
    async def on_auto_react(self, message: discord.Message):
        if len(message.mentions) == 0:
            return
        if message.guild.id not in self.backend.cach["react"].keys():
            return
        ...
        for mention in message.mentions:
            if mention.id in guild_data.keys():
                ...
                if user_data["last_react"] is None:
                    ...
                    await self.backend.update_cache(
                        self.backend.types.reacts, message.guild, user_data
                    )
                else:
                    if now > user_data["last_react"] + datetime.timedelta(seconds=5):
                        try:
                            await message.add_reaction(
                                random.choice(user_data["emojis"])
                            )
                        except Exception:
                            continue
                        user_data["last_react"] = datetime.datetime.utcnow()
