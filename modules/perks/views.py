...
profile_select_options = [SelectOption(label="Roles", value="roles"),SelectOption(label="Channels", value="channels"),SelectOption(label="Auto Reacts", value="reacts"),SelectOption(label="Highlighters", value="highlights"),SelectOption(label="Emojis", value="emojis")]
...
class PerkConfig(View):
    def __init__(self, user:discord.Member, data:Config, message: discord.Message=None):
        self.user = user
        self.data = data 
    ...
    @button(label="Admin Roles", style=discord.ButtonStyle.gray, emoji="<:tgk_admin:1073908306713780284>", row=1)
    async def admin_roles(self, interaction: Interaction, button: Button):
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(embed=discord.Embed(description=f"Added Roles: {', '.join([role.mention for role in add_roles]) if add_roles else '`None`'}\nRemoved Roles: {', '.join([role.mention for role in remove_roles]) if remove_roles else '`None`'}", color=interaction.client.default_color), delete_after=10)

            await interaction.client.Perk.update("config", self.data)
            await self.message.edit(embed=await interaction.client.Perk.get_config_embed(interaction.guild, self.data))
        
    @button(label="Custom Roles Position", style=discord.ButtonStyle.gray, emoji="<:tgk_role:1073908306713780284>", row=1)
    async def custom_roles_position(self, interaction: Interaction, button: Button):
        ...
        if view.value:
            ...
            await interaction.client.Perk.update("config", self.data)
            await view.select.interaction.response.edit_message(content=f"Custom roles position set to <@&{position}>", view=None)
            await self.message.edit(embed=await interaction.client.bot.Perk.get_config_embed(interaction.guild, self.data))
            await self.message.edit(embed=await self.update_embed(interaction, self.data))
    
    @button(label="Custom Category", style=discord.ButtonStyle.gray, emoji="<:tgk_category:1076602579846447184>",row=1)
    ...
    async def max_emojis(self, interaction: Interaction, button: Button):
        ...
        view.max_emojis = TextInput(label="Enter the max emojis",min_length=1, max_length=2, required=True)
    ...
    @button(label="Emoji Request Channel", style=discord.ButtonStyle.gray, emoji="<:tgk_emoji:1073908306713780284>", row=2)
    async def max_channels(self, interaction: Interaction, button: Button):
        view = View()
        view.value = False
        view.select = Channel_select(placeholder="Select a channel to set as the emoji request channel", max_values=1, min_values=1, channel_types=[discord.ChannelType.text])
        view.add_item(view.select)
        ...
        await interaction.client.Perk.update("config", self.data)
        await view.select.interaction.response.edit_message(content=f"Emoji request channel set to {view.select.values[0].mention}", view=None)
        await self.message.edit(embed=await interaction.client.Perk.get_config_embed(interaction.guild, self.data))

    @button(label="Profile", style=discord.ButtonStyle.gray, emoji="<:tgk_logging:1107652646887759973>", row=2)
    async def _profile(self, interaction: Interaction, button: Button):
        options = [
            SelectOption(label="List", description="List all the profiles", value="list", emoji="<:tgk_entries:1124995375548338176>"),
            SelectOption(label="Create", description="Create a new profile", value="create", emoji="<:tgk_color:1107261678204244038>"),
            SelectOption(label="Delete", description="Delete a profile", value="delete", emoji="<:tgk_delete:1113517803203461222>"),            
        ]

        view = View()
        view.value = False
        view.select = Select_General(interaction=interaction, options=options, placeholder="Select an operation")
        view.add_item(view.select)
        ...
        match view.select.values[0]:
            ...
            case "create":
                formated_args = await get_formated_embed(["Role", "Duration", "Share Limit", "Perk Type"])
                embed = discord.Embed(title="", description="", color=interaction.client.default_color)
                embed.description = ""

                embed.description += "<:tgk_color:1107261678204244038> `Create a new profile`\n\n"
                ...
                profile_create_view = Profile_Manage(self.user, message=interaction.message, new=True)
                await view.select.interaction.response.edit_message(embed=embed, view=profile_create_view)
                
                await profile_create_view.wait()
                await self.message.edit(embed=await interaction.client.Perk.get_config_embed(interaction.guild))
                self.data = await interaction.client.Perk.get_data("config", interaction.guild.id, interaction.user.id)
                return
            
            case "list":
                profile_select = Select_General(interaction=interaction, options=profile_select_options, placeholder="Select a profile type", max_values=1, min_values=1)

                profile_select_view = View()
                profile_select_view.value = False
                profile_select_view.add_item(profile_select)

                await view.select.interaction.response.edit_message(view=profile_select_view, content=None)
                await profile_select_view.wait()

                if profile_select_view.value is False or profile_select_view.value is None:
                    return await interaction.delete_original_response()
                ...
                if len(pages) == 0:
                    embed = discord.Embed(description="No profiles found", color=interaction.client.default_color)
                    await interaction.edit_original_response(embed=embed, view=None)
                    return
                await interaction.delete_original_response()
                await Paginator(profile_select.interaction, pages).start(embeded=True, quick_navigation=False, hidden=True)
                return
            ...
            case "delete":
                ...
                profile_delete = Select_General(interaction=interaction, options=options, placeholder="Select a profile to delete", max_values=len(options), min_values=1)
                profile_delete_view = View()
                profile_delete_view.value = False
                profile_delete_view.add_item(profile_delete)

                await profile_select.interaction.response.edit_message(view=profile_delete_view, content=None)
                await profile_delete_view.wait()
                
                if profile_delete_view.value is False or profile_delete_view.value is None:
                    return await interaction.delete_original_response()
                ...
                await interaction.client.Perk.update("config", self.data)
                await profile_delete.interaction.response.edit_message(content="Profile(s) deleted", view=None)
                await self.message.edit(embed=await interaction.client.Perk.get_config_embed(interaction.guild, self.data))
...
class Profile_Manage(View):
    def __init__(self, user: discord.Member, data: Profile=None, message: discord.Message=None, type:str = None,new=False):
        ...
        if self.new == True:
            ...
            self.data: Profile = {"role_id": None, "duration": "permanent", "share_limit": None}
    ...
    async def interaction_check(self, interaction: Interaction):
        if interaction.user.id == self.user.id:
            return True
        await interaction.response.send_message("You are not the owner of this profile", ephemeral=True)
        return False
    ...
    @select(placeholder="Select a Role for the profile", min_values=1, max_values=1, cls=discord.ui.RoleSelect, disabled=True)
    ...
    @select(placeholder="Select share limit for the profile", min_values=1, max_values=1, 
            options=[
                SelectOption(label=i, value=i) for i in range(1, 11)
            ])
    async def share_limit(self, interaction: Interaction, select: Select):
        self.data['share_limit'] = int(select.values[0])
        select.options = [SelectOption(label=select.values[0].capitalize(), value=select.values[0], default=True)]
        select.disabled = True
        await interaction.response.edit_message(view=self, embed=await self.get_embed(interaction, self.data))
    
    @select(placeholder="Select Perk Type", options=[
    ...
    ], max_values=1, min_values=1, disabled=False)
    ...
    async def save(self, interaction: Interaction, button: Button):
        if not all([self.children[0].disabled, self.children[1].disabled, self.children[2].disabled]):
            await interaction.response.send_message("You are missing some required fields", ephemeral=True)
            return
            
        config: PerkConfig = await interaction.client.Perk.get_data("config", interaction.guild.id, interaction.user.id)
        button.disabled = True
        if str(self.data['role_id']) in config['profiles'][self.type]:
            await interaction.response.send_message("A profile with the same role already exists", ephemeral=True)
        else:
        ...
        match self.type:
            case "roles":
                ...
                for value in select.values:
                    if value.id == self.data['user_id']: continue
...
class Emoji_Request(View):
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        config = await interaction.client.Perk.get_data("config", interaction.guild.id, interaction.user.id)
    ...
    @button(label="Approve", style=discord.ButtonStyle.gray, emoji="<:tgk_active:1082676793342951475>", custom_id="emoji:approve")
    async def approve(self, interaction: Interaction, button: Button):
        await interaction.response.send_message("Processing...", ephemeral=True)

        config: Config = await interaction.client.Perk.get_data("config", interaction.guild.id, interaction.user.id)
        emojis_data = await interaction.client.Perk.emoji_request.find(interaction.message.id)
        all_emojis = await interaction.client.Perk.emoji.find_many_by_custom({"guild_id": interaction.guild.id})
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(interaction.message.attachments[0].url) as resp:
                if resp.status != 200:
                    return await interaction.response.send_message("Failed to download the emoji", ephemeral=True)
                emoji_bytes = await resp.read()
                await session.close()

        user_profile = await interaction.client.Perk.calulate_profile(interaction.client.Perk.types.emojis, interaction.guild, interaction.user)
        user_data: Custom_Emoji = await interaction.client.Perk.get_data("emojis", interaction.guild.id, interaction.user.id)
        if not user_data:
            user_data = await interaction.client.Perk.create("emojis", user_id=emojis_data['user_id'], guild_id=interaction.guild.id, duration="permanent", share_limit=user_profile['share_limit'])        
        ...
        if len(user_data['emojis']) >= user_data['max_emoji']:
            ...
            await interaction.edit_original_response(content="User has reached the max emoji limit", view=None)
            await interaction.message.delete()
            try:
                await user.send("You emoji request has been rejected because you have reached the max emoji limit")
            except discord.HTTPException:
                pass
            return
        emoji_name = emojis_data['name'] if 'tgk_' in emojis_data['name'] else f"tgk_{emojis_data['name']}"
        emoji: discord.Emoji = await interaction.guild.create_custom_emoji(name=emoji_name, image=emoji_bytes, reason=f"Emoji Request Approved by {interaction.user.name}")
        ...
        await interaction.message.edit(content=f"Aproved by {interaction.user.mention}", view=self)

    @button(label="Reject", style=discord.ButtonStyle.gray, emoji="<:tgk_deactivated:1082676877468119110>", custom_id="emoji:reject")
    async def reject(self, interaction: Interaction, button: Button):
        emoji_data = await interaction.client.Perk.emoji_request.find(interaction.message.id)
        modal = General_Modal(title="Reject Reason", interaction=interaction)
        modal.reason = TextInput(label="Reason for rejecting the emoji", min_length=1, max_length=500, required=True, style=discord.TextStyle.paragraph)
        modal.add_item(modal.reason)
        await interaction.response.send_modal(modal)

        await modal.wait()
        if modal.reason.value:
            await modal.interaction.response.send_message("Processing...", ephemeral=True)
            user = interaction.guild.get_member(emoji_data['user_id'])
            
            try:
                await user.send(f"Your emoji request has been rejected for the following reason:\n{modal.reason.value}")
            except discord.HTTPException:
                pass
            await interaction.client.Perk.emoji_request.delete(emoji_data['_id'])
            await modal.interaction.edit_original_response(content="Emoji request rejected", view=None)
            await interaction.message.delete()
