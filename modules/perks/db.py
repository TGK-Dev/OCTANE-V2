...
class Perks_DB:
    ...
    async def get_data(self, type: Perk_Type | str, guild_id: int, user_id: int) -> Union[Custom_Roles, Custom_Channel, Custom_React, Custom_Highlight, Custom_Emoji, Config]:
        """
    ...
    async def create(self, type: Perk_Type | str, user_id: int, guild_id: int, duration: Union[int, str]=None, share_limit: int=None)  -> Union[Custom_Roles, Custom_Channel, Custom_React, Custom_Highlight, Custom_Emoji, Config]:
        """
        ...
        match type:
            case Perk_Type.roles | "roles":
                perk_data: Custom_Roles = {'user_id': user_id,'guild_id': guild_id,'role_id': None,'duration': duration,'created_at': None,'share_limit': share_limit,'friend_list': []}
            ...
            case Perk_Type.reacts | "reacts":
                perk_data: Custom_React = {'guild_id': guild_id, 'user_id': user_id, 'emojis': [], 'last_react': None, 'max_emoji': share_limit if share_limit else 1}
                await self.react.insert(perk_data)
                return perk_data

            case Perk_Type.highlights | "highlights":
                perk_data: Custom_Highlight = {'guild_id': guild_id, 'user_id': user_id, 'triggers': [], 'ignore_channel':[], 'ignore_users': [], 'last_trigger': None, 'tigger_limit': share_limit if share_limit else 1}
                await self.highlight.insert(perk_data)
                return perk_data
        
            case Perk_Type.emojis | "emojis":
                perk_data: Custom_Emoji = {'emojis': [], 'max_emoji': share_limit, 'guild_id': guild_id, 'user_id': user_id}
                await self.emoji.insert(perk_data)
                return perk_data
            
            case Perk_Type.config | "config":
                perk_config: Config = {'_id': guild_id,'custom_category': {'name': None, 'last_cat': None, 'cat_list': []},'custom_roles_position': 0, 'admin_roles': [], "emojis": {"max": 0, 'request_channel': None},"profiles": {"roles": {}, "channels": {}, "reacts": {}, "highlights": {}, "emojis": {}}}
    ...
    async def update_cache(self, perk: Perk_Type, guild: discord.Guild, data: dict) -> None:
        """
    ...
    async def get_config_embed(self, guild: discord.Guild, config: Config=None) -> discord.Embed:
        """
        ...
        profile_formated_args = await get_formated_embed(["Roles", "Channels", "Reacts", "Highlights", "Emojis"])
    ...
    async def calulate_profile(self, type: Perk_Type, guild: discord.Guild, user: discord.Member) -> dict:
        """
