...
from discord.ui import View, Button, Select, button, TextInput
...
class RoleMenu_Panel(View):
    ...
    async def update_embed(self):
        ...
        embed.add_field(
            name="Display Name",
            value=f"<:nat_reply:1146498277068517386> {self.profile['display_name']}",
            inline=False,
        )
        embed.add_field(
            name="Required Role",
            value=f"<:nat_reply:1146498277068517386> {req_role}",
            inline=False,
        )
        embed.add_field(
            name="Blacklisted Role",
            value=f"<:nat_reply:1146498277068517386> {bl_role}",
            inline=False,
        )
        embed.add_field(
            name="Type",
            value=f"<:nat_reply:1146498277068517386> {str(ReactRoleMenuType(self.profile['type']))}",
            inline=False,
        )
    ...
    async def _display_name(self, interaction: Interaction, button: Button):
        ...
        modal.text = TextInput(
            label="Enter a display name", placeholder="Display Name", max_length=100
        )
        ...
        await modal.interaction.response.send_message(
            content=f"Set display name to {modal.text.value}", view=None
        )
    ...
    async def _req_role(self, interaction: Interaction, button: Button):
        ...
        await view.select.interaction.response.edit_message(
            content=f"Added: {','.join(added) if len(added) != 0 else '`None`'}\nRemoved: {','.join(removed) if len(removed) != 0 else '`None`'}",
            view=None,
        )

        await self.update_embed()

        await interaction.client.rm.update_profile(
            self.guild.id, self.profile["name"], self.profile
        )
    ...
    async def _menu_type(self, interaction: Interaction, button: Button):
        ...
        view.select = Select_General(
            ...
            options=[
                SelectOption(
                    label=str(ReactRoleMenuType.ADD_ONLY),
                    value=ReactRoleMenuType.ADD_ONLY.value,
                ),
                SelectOption(
                    label=str(ReactRoleMenuType.REMOVE_ONLY),
                    value=ReactRoleMenuType.REMOVE_ONLY.value,
                ),
                SelectOption(
                    label=str(ReactRoleMenuType.ADD_AND_REMOVE),
                    value=ReactRoleMenuType.ADD_AND_REMOVE.value,
                ),
                SelectOption(
                    label=str(ReactRoleMenuType.UNIQUE),
                    value=ReactRoleMenuType.UNIQUE.value,
                ),
            ],
            max_values=1,
            min_values=1,
        ...
        await view.select.interaction.response.edit_message(
            content=f"Set type to {str(ReactRoleMenuType(self.profile['type']))}",
            view=None,
        )
    ...
    async def _manage_roles(self, interaction: Interaction, button: Button):
        ...
        op_view.select = Select_General(
            ...
            options=[
                ...
                SelectOption(
                    label="Clear Roles",
                    value="clear",
                    description="Clear all roles from the menu",
        ...
        match op_view.select.values[0]:
            case "add":
                ...
                embed = discord.Embed(
                    title="Add Role",
                    description="Select a role you want to add",
                ...
                await op_view.select.interaction.response.edit_message(
                    embed=embed, view=add_role
                )
                ...
                if add_role.value is None or False:
                    ...
                    return
                if (
                    interaction.user.top_role.position
                    < add_role.select.values[0].position
                ):
                    await add_role.select.interaction.response.send_message(
                        "You can't add roles higher than your top role", ephemeral=True
                    )
                    return
                if (
                    add_role.select.values[0].position
                    > interaction.guild.me.top_role.position
                ):
                    await add_role.select.interaction.response.send_message(
                        "I can't add roles higher than my top role", ephemeral=True
                    )
                    return

                if (
                    add_role.select.values[0].permissions.administrator
                    or add_role.select.values[0].permissions.manage_roles
                    or add_role.select.values[0].permissions.manage_guild
                    or add_role.select.values[0].permissions.ban_members
                    or add_role.select.values[0].permissions.kick_members
                ):
                    await add_role.select.interaction.response.send_message(
                        "Due to security reason you can't add this role", ephemeral=True
                    )
                    return

                data["role_id"] = add_role.select.values[0].id
                emoji_modal = General_Modal(
                    title="Emoji Selection", interaction=add_role.select.interaction
                )
                emoji_modal.value = None
                emoji_modal.emoji = TextInput(
                    label="Paste an emoji you want to use",
                    placeholder="it's recommended to use custom emojis",
                    max_length=100,
                )
                ...
                embed.description = ""
                embed.add_field(
                    name="Role", value=add_role.select.values[0].mention, inline=False
                )
                embed.add_field(
                    name="Emoji", value=emoji_modal.emoji.value, inline=False
                )
                ...
                try:
                    ...
                    await self.message.remove_reaction(
                        data["emoji"], interaction.guild.me
                    )
                except Exception:
                    await emoji_modal.interaction.response.edit_message(
                        content="Invalid emoji", view=None, embed=None
                    )
                    return
                await emoji_modal.interaction.response.edit_message(
                    content="Added role", view=None, embed=None
                )
            ...
            case "remove":
                ...
                for roles in self.profile["roles"].values():
                    ...
                    option.append(
                        SelectOption(
                            label=role.name, value=role.id, emoji=roles["emoji"]
                        )
                    )
                remove_view.select = Select_General(
                    interaction=interaction,
                ...
                await remove_view.select.interaction.response.edit_message(
                    content="Removed role(s)", view=None, embed=None
                )
            ...
            case "clear":
                ...
                await op_view.select.interaction.response.edit_message(
                    content="Cleared all roles", view=None, embed=None
                )
...
class RoleMenu_Button(Button):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if not role:
            await interaction.response.send_message(
                embed=discord.Embed(
                    description="Error: Role not Found contanct server admins",
        ...
        match menu["type"]:
            ...
            case ReactRoleMenuType.UNIQUE.value:
                ...
                removed_roles = ""
                for arole in roles:
                    if arole in interaction.user.roles:
                        await interaction.user.remove_roles(arole)
                        removed_roles += f"{arole.mention}, "
                await interaction.user.add_roles(role)
                added_roles += f"{role.mention}, "
                embed = discord.Embed(color=interaction.client.default_color)
                embed.description = (
                    f"Added Role(s): {added_roles}\nRemoved Role(s): {removed_roles}"
                )
            ...
            case _:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        description="Invalid menu type", color=discord.Color.red()
                    ),
                    ephemeral=True,
                )
                return
    ...
    async def callback(self, interaction: Interaction):
        ...
        match menu["type"]:
            ...
            case ReactRoleMenuType.REMOVE_ONLY.value:
                ...
                embed = discord.Embed(color=interaction.client.default_color)
                embed.add_field(
                    name="Removed Role(s)", value=removed_roles, inline=False
                )
                await interaction.followup.send(embed=embed, ephemeral=True)
                return
            ...
            case "dropdown":
                ...
                for role in self.menu["roles"].values():
                    ...
                    if _role:
                        option.append(
                            SelectOption(
                                label=_role.name, value=_role.id, emoji=role["emoji"]
                            )
                        )

                select = RoleMenu_Select(
                    placeholder="Tap here to modify your roles",
                    options=option,
                    max_values=len(option),
                    min_values=1,
                    custom_id=f"react_roles:{self.guild.id}:{self.menu['name']}",
                )
    ...
    async def interaction_check(self, interaction: Interaction):
        ...
        if set(user_role) & set(self.blacklisted_roles):
            ...
            for role in self.blacklisted_roles:
                bl_roles += f"<@&{role}>, "
            await interaction.response.send_message(
                "You have blacklisted roles to use this menu\n", ephemeral=True
            )
            return False
        ...
        if set(user_role) & set(self.requried_roles):
            ...
            for role in self.requried_roles:
                req_roles += f"<@&{role}>, "
            await interaction.response.send_message(
                f"You don't have the required roles to use this menu\nRequired Roles: {req_roles}",
                ephemeral=True,
            )
            return False
