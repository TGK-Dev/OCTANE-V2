...
from .db import Backend, RoleMenu, RoleMenuProfile, ReactRoleMenuType
...
class RoleMenus(commands.GroupCog, name="role_menus", description="Role menus"):
    ...
    async def on_ready(self):
        ...
        for key, value in self.backend.Cach.items():
            for profile in value["roles"].values():
                ...
                viewb = RoleMenu_Perent(
                    profile, self.bot.get_guild(key), _type="button"
                )
                viewd = RoleMenu_Perent(
                    profile, self.bot.get_guild(key), _type="dropdown"
                )
    ...
    async def create(self, interaction: discord.Interaction, display_name: str):
        ...
        if display_name in config["roles"].keys():
            await interaction.response.send_message(
                "A role menu with that name already exists", ephemeral=True
            )
            return
        if len(config["roles"]) >= config["max_profiles"]:
            await interaction.response.send_message(
                "You have reached the max amount of profiles", ephemeral=True
            )
            return
        ...
        embed = discord.Embed(
            title=f"Creating New Role Menu {display_name}",
            description="",
            color=interaction.client.default_color,
        )
        embed.add_field(
            name="Display Name",
            value=f"<:nat_reply:1146498277068517386> {profile['display_name']}",
            inline=True,
        )
        embed.add_field(name="Required Role", value="`None`", inline=True)
        embed.add_field(name="Blacklisted Role", value="`None`", inline=True)
        embed.add_field(
            name="Type", value=str(ReactRoleMenuType(profile["type"])), inline=True
        )
    ...
    async def edit(self, interaction: discord.Interaction, profile: str):
        ...
        if data is None:
            await interaction.response.send_message(
                "That profile does not exist", ephemeral=True
            )
            return
        profile = RoleMenuProfile(**data)
        embed = discord.Embed(
            title=f"Editing Role Menu {profile['display_name']}",
            description="",
            color=interaction.client.default_color,
        )
        ...
        embed.add_field(
            name="Type",
            value=f"<:nat_reply:1146498277068517386> {str(ReactRoleMenuType(profile['type']))}",
            inline=False,
        )
    ...
    async def delete(self, interaction: discord.Interaction, profile: str):
        ...
        await interaction.response.send_message(
            f"Deleted profile {profile['display_name']}", ephemeral=True
        )
    ...
    async def _list(self, interaction: discord.Interaction):
        ...
        if data is None:
            await interaction.response.send_message(
                "There are no profiles", ephemeral=True
            )
        pages = []
        for menu in data.values():
            embed = discord.Embed(
                title=f"Role Menu {menu['display_name']}",
                description="",
                color=interaction.client.default_color,
            )
            embed.description += "* **DisplayName**: " + menu["display_name"] + "\n"
            embed.description += (
                "* **Type**: " + str(ReactRoleMenuType(menu["type"])) + "\n"
            )
            req_role = []
    ...
    @app_commands.describe(
        profile="The profile to send",
        timeout="How long the menu should be active for",
        embed="The embed format",
        _type="The interaction type of the menu (button or dropdown)",
    )
    ...
    async def send(
        ...
        profile: str,
        timeout: app_commands.Transform[int, TimeConverter] = None,
        embed: str = "big",
        _type: str = "button",
    ):
        data = await self.backend.get_profile(interaction.guild_id, profile)
        ...
        match embed:
            case "big":
                view = RoleMenu_Perent(
                    data, interaction.guild, timeout, labled=False, _type=_type
                )
                ...
                embed.description += "Press the below buttons to interact with menu\n"
                embed.set_footer(
                    text=f"Menu type: {str(ReactRoleMenuType(profile['type']))}"
                )
            ...
            case "small":
                ...
                view = RoleMenu_Perent(
                    data, interaction.guild, timeout, labled=True, _type=_type
                )
