...
from .view import Panels, TicketControl, Refresh_Trancsript
...
class Ticket(commands.GroupCog, name="ticket"):
    ...
    async def PanelAutoComplete(
        ...
        if len(config["panels"]) == {}:
            ...
            panels = [
                app_commands.Choice(name=panel["name"], value=panel["name"])
                for panel in config["panels"].keys()
    ...
    async def send(self, interaction: Interaction):
        await interaction.response.send_message(
            "Please wait while I update the panel message", ephemeral=True
        )
        ...
        for name, panel in panels.items():
            if panel["channel"] is not None:
                ...
                panels_channels[config["default_channel"]].append(
                    config["panels"][name]
                )
        ...
        for channel, panels in panels_channels.items():
            ...
            embed.set_author(
                name=f"{interaction.guild.name} Ticket System",
                icon_url=interaction.guild.icon.url,
            )

            for panel in panels:
                panel: Panel = panel
                if panel["panel_message"] is not None and panel_message is None:
                    try:
                ...
                embed.add_field(
                    name=panel["name"], value=panel["description"], inline=False
                )

            view = Panels(
                panels=panels_channels[channel.id], guild_id=interaction.guild_id
            )
            self.bot.add_view(view)

            if panel_message is None and len(panels) > 0:
        ...
        if isinstance(target, discord.Role):
            if target.id in ticket_data["added_roles"]:
                return await ctx.send(
                    embed=discord.Embed(
                        description=f"The role {target.mention} already part of the ticket",
            ...
            if target.id in ticket_data["added_users"]:
                return await ctx.send(
                    embed=discord.Embed(
                        description=f"The user {target.mention} is already part of the ticket",
                        color=self.bot.default_color,
                    ),
                    ephemeral=True,
                )
    ...
    @commands.hybrid_command(
        name="remove", description="Remove a Roles/Users from the ticket"
    )
    @app_commands.default_permissions(manage_messages=True)
    @app_commands.describe(target="Role/User to remove")
    async def _remove(
        self, ctx: commands.Context, target: discord.Role | discord.Member
    ):
        ticket_data: TicketData = await self.backend.ticket.find(ctx.channel.id)
        if not ticket_data:
            return await ctx.send("This is not a ticket channel", ephemeral=True)
