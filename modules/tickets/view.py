...
from .db import TicketConfig, Panel, Ticket, Qestion
from typing import Dict, List


class TicketConfig_View(View):
    def __init__(
        self, user: discord.Member, data: TicketConfig, message: discord.Message = None
    ):
        self.user = user
        self.data = data
    ...
    async def _admin_role_select(self, interaction: Interaction, button: Button):
        ...
        view.select = Role_select(
            placeholder="Select the role you want to add/remove",
            min_values=1,
            max_values=10,
        )
        view.add_item(view.select)
        await interaction.response.send_message(
            view=view, ephemeral=True, delete_after=30
        )
        await view.wait()
        ...
        await view.select.interaction.response.edit_message(
            content=f"Updated the admin roles\n\nAdded Roles: {', '.join(updates['added_roles'])}\nRemoved Roles: {', '.join(updates['removed_roles'])}",
            view=None,
            delete_after=10,
        )
        ...
        await self.message.edit(
            embed=await interaction.client.tickets.get_config_embed(data=self.data)
        )
    ...
    async def _panel_select(self, interaction: Interaction, button: Button):
        ...
        match view.select.values[0]:
            ...
            case "edit":
                ...
                embed = await interaction.client.tickets.panel_embed(
                    interaction.guild, panel
                )
                await panel_select.select.interaction.response.edit_message(
                    embed=embed, view=panel_view
                )
                await panel_view.wait()
                if panel_view.value is False or panel_view.value is None:
            ...
            case "delete":
                ...
                panel_select.select = Select_General(
                    interaction=interaction,
                    placeholder="Select the panel you want to delete",
                    options=[
                        SelectOption(label=f"{k}", value=k)
                        for k in self.data["panels"].keys()
                    ],
...
class TicketPanel(View):
    ...
    @button(
        label="Active",
        style=discord.ButtonStyle.gray,
        emoji="<:toggle_on:1123932825956134912>",
    )
    async def _active(self, interaction: Interaction, button: Button):
        ...
        await interaction.response.edit_message(
            embed=await interaction.client.tickets.panel_embed(
                guild=interaction.guild, data=self.data
            ),
            view=self,
        )
    ...
    @button(
        label="Name",
        style=discord.ButtonStyle.gray,
        emoji="<:tgk_id:1107614303575613520>",
    )
    async def _name(self, interaction: Interaction, button: Button):
        ...
        modal.name = TextInput(
            label="Enter the name of the panel",
            placeholder="Enter the name of the panel you want to create",
            min_length=3,
            max_length=100,
        )
        if self.data.get("name") is not None:
            modal.name.default = self.data["name"]
        ...
        if modal.name.value in self.config["panels"].keys():
            await interaction.response.send_message(
                "Panel with this name already exists", ephemeral=True, delete_after=5
            )
            return
    ...
    async def _ticket_message(self, interaction: Interaction, button: Button):
        view = Panel_Message(
            name=self.data["name"],
            interaction=interaction,
            message=self.data.get("ticket_message"),
        )
        embed = await view.update_embed()
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
    ...
    async def _roles(self, interaction: Interaction, button: Button):
        view = Panel_Roles(
            data={
                "support_roles": self.data["support_roles"],
                "ping_role": self.data["ping_role"],
            },
            interaction=interaction,
        )
        embed = await view.update_embed()
    ...
    async def _channel(self, interaction: Interaction, button: Button):
        view = Panel_Channel(
            ...
            current_channel={
                ...
                "nameing_scheme": self.data["nameing_scheme"],
            },
            name=self.data["name"],
        )
        embed = await view.update_embed()
    ...
    async def _emoji(self, interaction: Interaction, button: Button):
        modal = General_Modal(
            title=f"Editing {self.data['name']}'s Emoji", interaction=interaction
        )
        modal.name = TextInput(
            label="Enter/Edit Emoji", min_length=1, max_length=100, required=True
        )
        modal.name.default = (
            self.data["emoji"] if self.data.get("emoji") is not None else ""
        )
        modal.add_item(modal.name)
        await interaction.response.send_modal(modal)
        await modal.wait()
...
class Panel_Question(View):
    ...
    async def on_error(
        self, interaction: Interaction[discord.Client], error: Exception
    ) -> None:
        try:
            await interaction.response.send_message(
                embed=discord.Embed(
    ...
    @button(
        label="Add Question",
        style=discord.ButtonStyle.gray,
        emoji="<:tgk_message_add:1073908702958059550>",
    )
    async def _add(self, interaction: Interaction, button: Button):
        ...
        if view.value is False or view.value is None:
            return await view.interaction.response.edit_message(
                content="Canceled the question", view=None, delete_after=5
            )

        self.data[str(len(self.data.keys()) + 1)] = {
            "question": modal.qes.value,
            "answer": modal.ans.value,
        }
        await view.interaction.response.edit_message(
            content="Added the question", view=None, delete_after=5
        )
    ...
    async def _edit(self, interaction: Interaction, button: Button):
        ...
        view.select = Select_General(
            interaction=interaction,
            placeholder="Select the question you want to edit",
            options=[
                SelectOption(label=f"{v['question']}", value=k)
        ...
        self.data[view.select.values[0]] = {
            "question": modal.qes.value,
            "answer": modal.ans.value,
        }
        await view.interaction.response.edit_message(
            content="Edited the question", view=None, delete_after=5
        )
    ...
    async def _delete(self, interaction: Interaction, button: Button):
        view = Select_General(
            interaction=interaction,
            placeholder="Select the question you want to delete",
            options=[
                SelectOption(label=f"{v['question']}", value=k)
                for k, v in self.data.items()
            ],
        ...
        question = self.data[view.select.values[0]]
        embed = discord.Embed(
            description=f"Are you sure you want to delete the question\n\n{question['question']}\n{question['answer']}",
            color=interaction.client.default_color,
        )
        view = Confirm(user=interaction.user, timeout=30)
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
        ...
        await view.interaction.response.edit_message(
            content="Deleted the question", view=None, delete_after=5
        )
    ...
    async def _confirm(self, interaction: Interaction, button: Button):
        ...
        for chl in self.children:
            chl.disabled = True
        await interaction.response.send_message(
            "Updated the panel questions", ephemeral=True, delete_after=10, view=self
        )
        self.stop()
...
class Panel_Message(View):
    ...
    async def _edit(self, interaction: Interaction, button: Button):
        modal = General_Modal(
            title=f"Editing {self.name}'s ticket message", interaction=interaction
        )
        modal.name = TextInput(
            label="Edit/Add the message",
            min_length=1,
            max_length=1000,
            required=True,
            style=discord.TextStyle.long,
        ...
        if self.message is not None:
            modal.name.default = self.message
        modal.add_item(modal.name)
        await interaction.response.send_modal(modal)
        await modal.wait()
        ...
        self.children[1].disabled = False
        await modal.interaction.response.edit_message(
            view=self, embed=await self.update_embed()
        )
...
class Panel_Roles(View):
    ...
    async def update_embed(self):
        ...
        embed.description += "<:tgk_role:1073908306713780284> `Panel Roles`\n\n"
    ...
    @select(
        placeholder="Select the role you want to add/remove from the panel",
    ...
    @select(
        placeholder="Select the role you want to ping when a ticket is created",
        cls=discord.ui.RoleSelect,
        min_values=1,
        max_values=1,
    )
    async def _ping_role(self, interaction: Interaction, select: Select):
        self.data["ping_role"] = (
            select.values[0].id
            if self.data["ping_role"] != select.values[0].id
            else None
        )
        await interaction.response.edit_message(
            embed=await self.update_embed(), view=self
    ...
    async def _confirm(self, interaction: Interaction, button: Button):
        ...
        for chl in self.children:
            chl.disabled = True
        await interaction.response.edit_message(
            content="Updated the panel roles", delete_after=10, view=self
        )
        self.stop()
...
class Description(View):
    ...
    async def _edit(self, interaction: Interaction, button: Button):
        modal = General_Modal(
            title=f"Editing {self.name} Panel's Description", interaction=interaction
        )
        modal.name = TextInput(
            label="Enter/Edit Description",
            min_length=1,
            max_length=500,
            required=True,
            style=discord.TextStyle.long,
        ...
        if self.description is not None:
            modal.name.default = self.description
        modal.add_item(modal.name)
        await interaction.response.send_modal(modal)
        await modal.wait()
...
class Panel_Channel(View):
    ...
    @select(
        placeholder="Select the channel you want to use as the panel channel",
    ...
    @select(
        placeholder="Select the category you want to use as the panel category",
        cls=discord.ui.ChannelSelect,
        min_values=1,
        max_values=1,
    ...
    async def _nameing_scheme(self, interaction: Interaction, button: Button):
        ...
        if self.data.get("nameing_scheme") is not None:
            modal.name.default = self.data["nameing_scheme"]
        modal.add_item(modal.name)
    ...
    async def _confirm(self, interaction: Interaction, button: Button):
        ...
        for btn in self.children:
            btn.disabled = True
        await interaction.response.edit_message(
            content="Updated the panel channel", delete_after=5
        )
        self.stop()
...
class Channel_Config(View):
    ...
    @select(
        placeholder="Select Default Ticket Category",
        cls=discord.ui.ChannelSelect,
        min_values=1,
        max_values=1,
        channel_types=[discord.ChannelType.category],
    ...
    @select(
        placeholder="Select Default Ticket Panel Channel",
        cls=discord.ui.ChannelSelect,
        min_values=1,
        max_values=1,
        channel_types=[discord.ChannelType.text],
    ...
    async def _nameing_scheme(self, interaction: Interaction, button: Button):
        ...
        modal.open.default = (
            self.data["nameing_scheme"]["unlocked"]
            if self.data["nameing_scheme"].get("unlocked") is not None
            else ""
        )
        modal.close.default = (
            self.data["nameing_scheme"]["locked"]
            if self.data["nameing_scheme"].get("locked") is not None
            else ""
        )
    ...
    def __init__(self, panels: Dict[str, Panel] | List[Panel], guild_id: int):
        ...
        if isinstance(panels, dict):
            for panel in self.panels:
                self.add_item(
                    PanelButton(
                        custom_id=f"{guild_id}:panel:{panels[panel]['name']}",
                        label=panels[panel]["name"],
                        emoji=panels[panel]["emoji"],
                    )
                )
            ...
            for panel in self.panels:
                self.add_item(
                    PanelButton(
                        custom_id=f"{guild_id}:panel:{panel['name']}",
                        label=panel["name"],
                        emoji=panel["emoji"],
                    )
                )
...
class TicketQestionDropDown(View):
    def __init__(self, data: Qestion):
        ...
        self.qestions = [
            SelectOption(label=f"{value['question']}", value=key)
    ...
    @select(
        placeholder="Dose the following question apply to you?",
        min_values=1,
        max_values=1,
        options=[None],
    )
    async def _question(self, interaction: Interaction, select: Select):
        if select.values[0] != "none":
            ...
            embed.add_field(
                name=f"Q. {qestion['question']}",
                value=f"{qestion['answer']}",
                inline=False,
            )
...
class PanelButton(discord.ui.Button):
    ...
    async def callback(self, interaction: Interaction):
        config: TicketConfig = await interaction.client.tickets.get_config(
        ...
        try:
            ...
            return await interaction.response.send_message(
                "This panel does not exist", ephemeral=True
            )
        ...
        if panel["active"] is False:
            self.disabled = True
            await interaction.response.send_message(
                "This ticket panel is disabled by the administrator", ephemeral=True
            )
        ...
        if panel["name"].lower() == "partnership":
            ...
            if modal.data == {}:
                ...
                try:
                    ...
                    return await modal.interaction.edit_original_response(
                        content="<:tgk_deactivated:1082676877468119110> The invite you provided is invalid, please try again",
                        view=None,
                    )
                extra_content["pinfo"] = ""
                extra_content["pinfo"] += (
            ...
            await interaction.response.send_message(
                view=QestionView, ephemeral=True, delete_after=600
            )
        ...
        TicketName = (
            f"{panel['nameing_scheme']}{interaction.user.name}"
            if panel["nameing_scheme"] is not None
            else f"{config['nameing_scheme']['unlocked']} {interaction.user.name}"
        )
        ...
        TicketEmbed = discord.Embed(
            title=f"{panel['name']} Ticket",
            description=panel["ticket_message"],
            color=interaction.client.default_color,
        )
        TicketEmbed.set_footer(text=f"Ticket ID: {TicketChannel.id}")
        TicketMessage = await TicketChannel.send(
            embed=TicketEmbed,
            content=f"{interaction.user.mention} {interaction.guild.get_role(panel['ping_role']).mention if panel['ping_role'] is not None else ''}",
            view=TicketControl(),
        )
        ...
        view.add_item(
            discord.ui.Button(
                ...
                url=TicketMessage.jump_url,
            )
        )
        await interaction.edit_original_response(
            content=f"Ticket created in {TicketChannel.mention}", view=view
        )
...
class TicketControl(View):
    ...
    @button(
        label="Open",
        style=discord.ButtonStyle.gray,
        emoji="<:tgk_unlock:1072851439161983028>",
        custom_id="ticket:open",
    )
    async def _open(self, interaction: Interaction, button: Button):
        ticket_data: Ticket = await interaction.client.tickets.ticket.find(
        ...
        embed = discord.Embed(
            description="<a:loading:998834454292344842> Opening the ticket",
            color=interaction.client.default_color,
        )
        await interaction.response.send_message(embed=embed)
        overwrite = discord.PermissionOverwrite(view_channel=True)

        for role in ticket_data["added_roles"]:
            await interaction.channel.set_permissions(
                interaction.guild.get_role(role), overwrite=overwrite
            )
        for user in ticket_data["added_users"]:
            await interaction.channel.set_permissions(
                interaction.guild.get_member(user), overwrite=overwrite
            )
        ...
        if not ticket_owner:
            return await interaction.edit_original_response(
                embed=discord.Embed(
                    description="Unable to find the ticket owner in the server, they might have left the server",
                    color=discord.Color.red(),
                )
            )
        ...
        name = (
            f"{config['panels'][ticket_data['panel']]['nameing_scheme']}{ticket_owner.name}"
            if config["panels"][ticket_data["panel"]]["nameing_scheme"] is not None
            else f"{config['nameing_scheme']['unlocked']}{ticket_owner.name}"
        )

        await interaction.channel.edit(name=name)
        await interaction.edit_original_response(
            embed=discord.Embed(
                description=f"Ticket opended by the {interaction.user.mention}",
                color=interaction.client.default_color,
            )
        )
    ...
    async def _close(self, interaction: Interaction, button: Button):
        ...
        embed = discord.Embed(
            description="<a:loading:998834454292344842> Closing the ticket",
            color=interaction.client.default_color,
        )
        await interaction.response.send_message(embed=embed)
        ...
        await interaction.edit_original_response(
            embed=discord.Embed(
                description=f"Ticket closed by the {interaction.user.mention}",
                color=interaction.client.default_color,
            )
        )
    ...
    async def _delete(self, interaction: Interaction, button: Button):
        ...
        embed = discord.Embed(
            description=f"Deleting ticket in <t:{round(timestemp.timestamp())}:R>, {interaction.user.mention} use button below to cancel",
            color=interaction.client.default_color,
        )
        ...
        await interaction.response.send_message(
            embed=embed, view=ConfimView, ephemeral=False
        )
        ConfimView.message = await interaction.original_response()
        await ConfimView.wait()

        if ConfimView.value is True:
            return await ConfimView.interaction.response.edit_message(
                embed=discord.Embed(
                    description="Ticket deletion canceled",
                    color=interaction.client.default_color,
                ),
                view=None,
            )
        ...
        if Config["transcript_channel"] is None or not isinstance(
            TranscriptChannel, discord.TextChannel
        ):
        ...
        TranscriptFile = await chat_exporter.raw_export(
        ...
        TranscriptFile = discord.File(
            io.BytesIO(TranscriptFile.encode()),
            filename=f"transcript-{ticket_owner.name}-{ticket_data['panel']}.html",
        )
        ...
        TranscriptUrl = urllib.parse.quote(
            TranscriptMessage.attachments[0].url, safe=""
        )
    ...
    @button(
        label="Annonymous",
        style=discord.ButtonStyle.gray,
        emoji="<:tgk_amongUs:1103542462628253726>",
        custom_id="ticket:anonymous",
    )
    async def _anonymous(self, interaction: Interaction, button: Button):
        if not interaction.user.guild_permissions.ban_members:
            return await interaction.response.send_message(
                "You need to have `Ban Members` permission to use this button",
                ephemeral=True,
            )
        ticket_data: Ticket = await interaction.client.tickets.ticket.find(
            {"_id": interaction.channel.id}
        ...
        if ticket_data["anonymous"]["status"] is True:
            thread = interaction.guild.get_thread(ticket_data["anonymous"]["thread_id"])
            await thread.add_user(interaction.user)
            await interaction.response.send_message(
                embed=discord.Embed(
                    description="Added you to the tickets private thread",
                    color=interaction.client.default_color,
                ),
                ephemeral=True,
            )
            ...
            await thread.send(
                "please be advised that this thread is now designated as private and restricted only to moderators and higher-ups. Thank you for your understanding and cooperation in respecting the privacy of this thread."
            )
            await thread.add_user(interaction.user)
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    description="Created a private thread for the ticket",
                    color=interaction.client.default_color,
                ),
                ephemeral=True,
            )
...
class Refresh_Trancsript(View):
    ...
    async def refresh(self, interaction: Interaction, button: Button):
        if len(interaction.message.attachments) <= 0:
            await interaction.response.send_message(
                "No transcript found", ephemeral=True, delete_after=5
            )
...
class PartnerShipModal(discord.ui.Modal):
    def __init__(self, interaction: Interaction):
        ...
        self.data = {}
        super().__init__(
            timeout=120, title=f"Partnership form for {interaction.guild.name}"
        )

    server = TextInput(
        label="Enter the server name", min_length=1, max_length=100, required=True
    )
    invite = TextInput(
        label="Enter the server invite", min_length=1, max_length=100, required=True
    )
    ptype = TextInput(
        label="Enter the partnership type", min_length=1, max_length=100, required=True
    )
