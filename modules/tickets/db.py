...
class TicketDB:
    ...
    async def get_config_embed(
        self, _id: int = None, data: TicketConfig = None
    ) -> discord.Embed:
        """
        ...
        if _id:
            data = await self.get_config(_id)

        guild = self.bot.get_guild(data["_id"])
        if not guild:
            raise ValueError("Guild not found")
        embed_arg = await get_formated_embed(
            [
                "Admin Roles",
                "Default Category",
                "Log Channel",
                "Transcript Channel",
                "Default Panel Channel",
                "Panel Count",
            ]
        )
        ...
        embed.description = ""
        embed.description += (
            "<:tgk_message:1113527047373979668> `Ticket General Config`\n\n"
        )
    ...
    async def panel_embed(self, guild: discord.Guild, data: Panel) -> discord.Embed:
        ...
        embed.description += f"{embed_args['Description']} {'<:tgk_active:1082676793342951475>' if data['description'] is None else '<:tgk_deactivated:1082676877468119110>'}\n"
        embed.description += f"{embed_args['Ticket Message']} {'<:tgk_active:1082676793342951475>' if data['ticket_message'] is None else '<:tgk_deactivated:1082676877468119110>'}\n"
