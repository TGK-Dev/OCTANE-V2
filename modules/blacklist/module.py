...
from .db import Profile, Config, Blacklist, StrikeUser, backend
...
class Blacklist_cog(commands.GroupCog, name="blacklist"):
    ...
    async def profile_aucto_normal(
    ...
    async def unblacklist(self):
        ...
        for blacklist in data:
            if blacklist["Blacklist_end"] < now:
                ...
                if not user:
                    blacklist["Blacklist_end"] = now + datetime.timedelta(
                        seconds=blacklist["Blacklist_duration"]
                    )
    ...
    async def check_strike(self):
        ...
        for guild in guilds:
            ...
            strikes = await self.backend.strike.find_many_by_custom(
                {"guild_id": guild.id}
            )
            ...
            for user in strikes:
                ...
                if not member:
                    for strike in user["strikes"]:
                        strike["Strike_expire"] = now + datetime.timedelta(
                            seconds=profile["strike_expire"]
                        )
    ...
    async def on_blacklist_remove(self, blacklist: dict):
        ...
        if config["log_channel"]:
            ...
            embed = discord.Embed(
                title="Blacklist expired", color=self.bot.default_color, description=""
            )
        ...
        await self.backend.blacklist.delete(
            {"user_id": user.id, "profile": profile["_id"], "guild_id": guild.id}
        )
    ...
    @app_commands.describe(
        profile="Profile to apply blacklist",
        user="User to blacklist",
        reason="Reason for blacklist",
        duration="Duration of blacklist",
    )
    ...
    async def user_add(
        ...
        if profile not in config["profiles"].keys():
            return await interaction.response.send_message(
                f"Profile `{profile}` not found", ephemeral=True
            )
        ...
        if user_data is not None:
            return await interaction.response.send_message(
                f"{user.mention} is already blacklisted in profile `{profile}`",
                ephemeral=True,
            )
        ...
        user_data: Blacklist = {
            ...
            "Blacklist_end": datetime.datetime.utcnow()
            + datetime.timedelta(seconds=duration),
        }
        await self.backend.insert_blacklist(user_data)

        role_add = [
            interaction.guild.get_role(role_id) for role_id in profile_data["role_add"]
        ]
        role_remove = [
            interaction.guild.get_role(role_id)
            for role_id in profile_data["role_remove"]
        ]
        await user.add_roles(
            *role_add, reason=f"Blacklist by {interaction.user} ({interaction.user.id})"
        )
        await user.remove_roles(
            *role_remove,
            reason=f"Blacklist by {interaction.user} ({interaction.user.id})",
        )

        await interaction.response.send_message(
            f"{user.mention} has been blacklisted in profile `{profile}`",
            ephemeral=True,
        )
        ...
        if config:
            ...
            embed = discord.Embed(
                title="Blacklist", description="", color=discord.Color.red()
            )
            embed.description += f"**User:** {user.mention} ({user.id})\n"
    ...
    @app_commands.describe(
        profile="Profile to remove blacklist",
        user="User to remove blacklist",
        reason="Reason for removing blacklist",
    )
    @app_commands.autocomplete(profile=profile_aucto_normal)
    async def user_remove(
        ...
        if user_data is None:
            return await interaction.response.send_message(
                f"{user.mention} is not blacklisted in profile `{profile}`",
                ephemeral=True,
            )
        ...
        await user.add_roles(
            *role_add,
            reason=f"Blacklist removed by {interaction.user} ({interaction.user.id})",
        )
        await user.remove_roles(
            *role_remove,
            reason=f"Blacklist removed by {interaction.user} ({interaction.user.id})",
        ...
        await interaction.response.send_message(
            f"{user.mention} has been removed from blacklist in profile `{profile}`",
            ephemeral=True,
        )
        await self.backend.blacklist.delete(
            {
    ...
    async def _view(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if config is None:
            return await interaction.response.send_message(
                "This server doesn't have blacklist", ephemeral=True
            )
        pages = []
        for blacklist in await self.backend.blacklist.find_many_by_custom(
            {"user_id": user.id, "guild_id": interaction.guild_id}
        ):
            profile: Profile = config["profiles"][blacklist["profile"]]
            embed = discord.Embed(
                title=f"Blacklist of {user}",
            ...
            embed.description += (
                f"**Duration:** {format_timespan(blacklist['Blacklist_duration'])}\n"
            )
            embed.description += (
                f"**End:** <t:{int(blacklist['Blacklist_end'].timestamp())}:R>\n"
            )
            embed.description += f"**By:** <@{blacklist['Blacklist_by']}> ({blacklist['Blacklist_by']})\n"
            pages.append(embed)
        if len(pages) == 0:
    ...
    async def _strick_add(
        ...
        strike: StrikeUser = await self.backend.strike.find(
            {
                "user_id": user.id,
                "guild_id": interaction.guild_id,
                "profile": profile_data["_id"],
        ...
        strike["strikes"].append(
            {
                ...
                "Strike_expire": datetime.datetime.utcnow()
                + datetime.timedelta(seconds=profile_data["strike_expire"]),
            }
        )

        await self.backend.strike.upsert(strike)
        self.bot.dispatch("strike", user, interaction.user, profile_data["_id"], reason)

        if len(strike["strikes"]) >= profile_data["strike_limit"]:
            await interaction.edit_original_response(
                content=f"{user.mention} has reached strike limit in profile `{profile}` applying blacklist..."
            )
            ...
            role_add = [
                interaction.guild.get_role(role_id)
                for role_id in profile_data["role_add"]
            ]
            role_remove = [
                interaction.guild.get_role(role_id)
                for role_id in profile_data["role_remove"]
            ...
            await interaction.edit_original_response(
                content=f"{user.mention} has been blacklisted in profile `{profile}`"
            )

            try:
                embed = discord.Embed(
                    title=f"You have been blacklisted in profile {profile}",
                    description="",
                    color=self.bot.default_color,
                )
                if isinstance(user_blacklist["Blacklist_reason"], str):
                    embed.description += "**Reason:** Reached strike limit\n"
            ...
            if config["log_channel"]:
                ...
                embed.description += (
                    f"**Duration:** {format_timespan(profile_data['strike_expire'])}\n"
                )
            ...
            await interaction.edit_original_response(
                content=f"{user.mention} has been given strike in profile `{profile}` ({len(strike['strikes'])}/{profile_data['strike_limit']})"
            )
            try:
                embed = discord.Embed(
                    title=f"You have been given a strike in profile {profile}",
                    description="",
                    color=self.bot.default_color,
                )
                embed.description += f"**Reason:** {reason}\n"
                ...
                embed.set_footer(
                    text="Reaching strike limit will automatically blacklist you"
                )
                await user.send(embed=embed)
    ...
    @app_commands.describe(
        profile="Profile to remove strike", user="User to remove strike"
    )
    @app_commands.autocomplete(profile=profile_auto_strike)
    async def _strick_remove(
        self, interaction: discord.Interaction, user: discord.Member, profile: str
    ):
        ...
        user_data = await self.backend.strike.find(
            {"user_id": user.id, "guild_id": interaction.guild_id, "profile": profile}
        )
        if user_data is None:
            return await interaction.response.send_message(
                f"{user.mention} has no strike in profile `{profile}`", ephemeral=True
            )

        embed = discord.Embed(
            title=f"Strikes of {user}", color=self.bot.default_color, description=""
        )
        embed.description += f"**Profile:** {profile}\n"
        embed.description += (
            "**Total Strikes:** "
        ...
        view.select = Select_General(
            options=options,
            placeholder="Select strike to remove",
            min_values=1,
        ...
        await view.select.interaction.response.send_message(
            "Removing strike...", ephemeral=True
        )
        ...
        await interaction.followup.send(
            content=f"Suscessfully removed {len(view.select.values)} strike from {user.mention} in profile `{profile}`"
        )

    @app_commands.command(name="viewstrikes", description="View your strikes")
    async def _viewmystrike(
        self, interaction: discord.Interaction, user: discord.Member = None
    ):
        if user is not None:
            profiles: List[StrikeUser] = await self.backend.strike.find_many_by_custom(
                {"user_id": user.id, "guild_id": interaction.guild_id}
            )
        else:
            profiles: List[StrikeUser] = await self.backend.strike.find_many_by_custom(
        ...
        if len(profiles) == 0:
            return await interaction.response.send_message(
                "You don't have any active strikes", ephemeral=True
            )
        ...
        for profile in profiles:
            ...
            embed = discord.Embed(
                title=f"Strikes in profile {profile['profile']}",
                color=self.bot.default_color,
                description="",
            )
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        for blacklist in await self.backend.blacklist.find_many_by_custom(
            ...
            role_remove = [
                member.guild.get_role(role_id) for role_id in profile.role_remove
            ]

            await member.add_roles(
                *role_add,
                reason=f"Presistent blacklist by {member.guild.me} ({member.guild.me.id})",
            )
            await member.remove_roles(
                *role_remove,
                reason=f"Presistent blacklist by {member.guild.me} ({member.guild.me.id})",
            )
            blacklist["Blacklist_end"] = (
                datetime.datetime.utcnow()
                + datetime.timedelta(seconds=blacklist["Blacklist_duration"])
            )
    ...
    async def on_strike(
        ...
        config = await self.backend.get_config(member.guild.id)
        embed = discord.Embed(
            title="Strike", color=self.bot.default_color, description=""
        )
        embed.description += f"**User:** {member.mention} ({member.id})\n"
        embed.description += f"**Profile:** {profile}\n"
        embed.description += f"**Reason:** {reason}\n"
        embed.description += f"**By:** {moderator.mention} ({moderator.id})\n"
        if config["log_channel"]:
            channel = member.guild.get_channel(config["log_channel"])
            await channel.send(embed=embed)

    @commands.Cog.listener()
    async def on_blacklist(
        self,
        member: discord.Member,
        moderator: discord.Member,
        profile: str,
        ...
        config = await self.backend.get_config(member.guild.id)
        embed = discord.Embed(
            title="Blacklist", color=self.bot.default_color, description=""
        )
        embed.description += f"**User:** {member.mention} ({member.id})\n"
        embed.description += f"**Profile:** {profile}\n"
        embed.description += f"**Reason:** {reason}\n"
