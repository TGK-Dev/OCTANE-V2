...
class Emoji_Select(View):
    def __init__(self, user: discord.Member, guild: discord.Guild, type: Literal['animated', 'static']='static'):
        self.user = user
        self.guild = guild
        self.type = type
        self.current_page = 0
    ...
    def get_emojis(self, type: Literal['animated', 'static'], index: int):
        ...
        static_emojis = [static_emojis[i:i+24] for i in range(0, len(static_emojis), 24)]
        animated_emojis = [animated_emojis[i:i+24] for i in range(0, len(animated_emojis), 24)]
    ...
    def create_options(self, emojis: list):
        ...
        for emoji in emojis:
            options.append(SelectOption(label=emoji.name, value=emoji.id, emoji=str(emoji)))
        return options[:24]

    @select(placeholder="Select an emoji", min_values=1, max_values=1, options=[SelectOption(label="Animated", value="animated", emoji="🎥"), SelectOption(label="Static", value="static", emoji="🖼️")])
    async def _type(self, interaction: Interaction, select: Select):
        pass

    @button(style=discord.ButtonStyle.gray, emoji="<:tgk_rightarrow:1088526714205917325>")
    ...
    @button(label="Change Type", style=discord.ButtonStyle.gray, emoji="<:tgk_refresh:1171330950416842824>")
    async def _change_type(self, interaction: Interaction, button: discord.Button):
        pass

    @button(style=discord.ButtonStyle.gray, emoji="<:tgk_leftarrow:1088526575781285929>")
