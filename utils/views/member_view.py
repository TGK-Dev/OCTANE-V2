...
class Member_view(discord.ui.View):
    ...
    async def interaction_check(self, interaction: Interaction):
        if interaction.user.id == self.author.id:
            ...
            await interaction.response.send_message(
                "You are not the owner of this command", ephemeral=True
            )
            return False
    ...
    @discord.ui.select(
        ...
        custom_id="MEMBER_VIEW",
        max_values=1,
        options=[
            discord.SelectOption(label="Profile", value="profile"),
            discord.SelectOption(
                label="Badge", value="badge", emoji="<:bage:991740849664819332>"
            ),
            discord.SelectOption(
                label="Roles", value="roles", emoji="<:tgk_role:1073908306713780284>"
            ),
    ...
    async def member_view(
        ...
        match choice:
            case "profile":
                embed = discord.Embed(
                    title=f"User Info - {self.member.name}#{self.member.discriminator}"
                )
                embed.set_thumbnail(
                ...
                embed.add_field(
                    name="<:authorized:991735095587254364> ID:", value=self.member.id
                )
                embed.add_field(
                    name="<:displayname:991733326857654312> Display Name:",
                    value=self.member.display_name,
                )

                embed.add_field(
                    name="<:bot:991733628935610388> Bot Account:", value=self.member.bot
                )

                embed.add_field(
                    name="<:settings:991733871118917683> Account creation:",
                    value=self.member.created_at.strftime("%d/%m/%Y %H:%M:%S"),
                )
                embed.add_field(
                    name="<:join:991733999477203054> Server join:",
                    value=self.member.joined_at.strftime("%d/%m/%Y %H:%M:%S"),
                )
                await interaction.response.edit_message(embed=embed)

            case "badge":
                embed = discord.Embed(
                    title=f"Badges - {self.member.name}#{self.member.discriminator}"
                )
                badge = ""
                ...
                if (
                    discord.utils.get(self.member.guild.roles, id=818129661325869058)
                    in self.member.roles
                ):
                ...
                if (
                    discord.utils.get(self.member.guild.roles, id=931072410365607946)
                    in self.member.roles
                ):
                    badge += "First 50 Members  | <a:real_og:992048421974315051>\n"

                if (
                    discord.utils.get(self.member.guild.roles, id=786884615192313866)
                    in self.member.roles
                ):
            ...
            case "roles":
                embed = discord.Embed(
                    title=f"Roles - {self.member.name}#{self.member.discriminator}",
                    color=interaction.client.default_color,
                )
                roles = ""
            ...
            case "votes":
                embed = discord.Embed(
                    title=f"Votes - {self.member.name}#{self.member.discriminator}",
                    color=interaction.client.default_color,
                )
