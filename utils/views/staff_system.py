...
class Staff_config_edit(View):
    ...
    async def update_embed(self, data: dict, interaction: Interaction):
        ...
        embed.description += (
            f"**Owners:** {', '.join([f'<@{owner}>' for owner in data['owners']])}\n"
        )
        embed.description += f"**Staff Manager:** {', '.join([f'<@{manager}>' for manager in data['staff_manager']]) if data['staff_manager'] else '`None`'}\n"
        embed.description += (
            "**Base Role:**"
            + (f" <@&{data['base_role']}>" if data["base_role"] is not None else "`None`")
            + "\n"
        )
        embed.description += (
            "**Leave Role:**"
            + (f" <@&{data['leave_role']}>" if data["leave_role"] is not None else "`None`")
            + "\n"
        )
        embed.description += (
            "**Leave Channel:**"
            + (
                f" <#{data['leave_channel']}>"
                if data["leave_channel"] is not None
                else "`None`"
            )
            + "\n"
        )
        embed.description += (
            "**Logging Channel:**"
            + (" **Webhook setup done**" if data["webhook_url"] is not None else "`None`")
            + "\n"
        )
        embed.description += f"**Max Positions:** {data['max_positions']}\n"
        embed.description += (
            "**Last Edit:** " + f"<t:{round(data['last_edit'].timestamp())}:R>\n"
            if data["last_edit"]
            else "`None`\n"
        )
    ...
    @button(
        ...
        emoji="<:tgk_owner:1073588580796092558>",
        row=0,
    )
    async def owners(self, interaction: Interaction, button: Button):
        ...
        view.select = User_Select(
            placeholder="Please select the owners you want to add/remove",
            min_values=1,
            max_values=5,
        )
        ...
        if view.value:
            ...
            await interaction.client.staff_db.update_config(
                interaction.guild.id, self.data
            )
            await view.select.interaction.response.edit_message(
                embed=discord.Embed(
                    description=f"**Added Owners:** {addeed_owners}\n**Removed Owners:** {removed_owners}",
                    color=0x2B2D31,
                ),
                view=None,
            )
            await self.message.edit(
                embed=await self.update_embed(self.data, interaction)
            )
    ...
    async def staff_manager(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        view.select = User_Select(
            placeholder="Please select the users you want to add/remove",
            min_values=1,
            max_values=5,
        )
        view.add_item(view.select)
    ...
    async def base_role(self, interaction: Interaction, button: Button):
        ...
        view.select = Role_select(
            placeholder="Please select the base role", min_values=1, max_values=1
        )
        view.add_item(view.select)
        await interaction.response.send_message(
            view=view, ephemeral=True, delete_after=30
    ...
    async def leave_role(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        view.select = Role_select(
            placeholder="Please select the leave role", min_values=1, max_values=1
        )
        view.add_item(view.select)
        await interaction.response.send_message(
            view=view, ephemeral=True, delete_after=30
    ...
    async def leave_channel(self, interaction: Interaction, button: Button):
        ...
        view.select = Channel_select(
            placeholder="Please select the leave channel",
            min_values=1,
            max_values=1,
    ...
    async def logging(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        view.select = Channel_select(
            placeholder="Please select the logging channel",
            min_values=1,
            max_values=1,
            channel_types=[discord.ChannelType.text],
        )
        ...
        if view.value:
            ...
            webhook = await channel.create_webhook(
                name=f"{interaction.client.user.name} Logging"
            )
    ...
    async def positions(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        options = [
            SelectOption(label="Add Position", value="add"),
            SelectOption(label="Remove Position", value="remove"),
        ]
        ...
        if view.value:
            match view.select.values[0]:
                case "add":
                    ...
                    modal = General_Modal(
                        title="New Position Creating form",
                        interaction=view.select.interaction,
                    )
                    modal.question = TextInput(
                        label="Enter the name of the position",
                    ...
                    if modal.value:
                        if modal.question.value in self.data["positions"].keys():
                            return (
                                await modal.question.interaction.response.edit_message(
                                    embed=discord.Embed(
                        ...
                        view.value = False
                        view.select = Role_select(
                            placeholder="Please select the role for the position",
                            min_values=1,
                            max_values=1,
                        )
                        view.add_item(view.select)
                        ...
                        if view.value:
                            ...
                            await view.select.interaction.response.edit_message(
                                embed=discord.Embed(
                                    description=f"**Added Position:** `{position['name'].capitalize()}`\n**Role:** <@&{position['role']}>\n**Owner Only:** {position['owner_only']}",
                                    color=0x2B2D31,
                                ),
                                view=None,
                            )
                ...
                case "remove":
                    ...
                    delete_view.select = Select_General(
                        placeholder="Please select the position you want to remove",
                        options=[
                            SelectOption(label=position.capitalize(), value=position)
                            for position in self.data["positions"].keys()
                        ],
                        max_values=1,
                    )
                    ...
                    if delete_view.value:
                        ...
                        await delete_view.select.interaction.response.edit_message(
                            embed=discord.Embed(
                                description=f"**Removed Position:** `{position.capitalize()}`",
                                color=0x2B2D31,
                            ),
                            view=None,
                        )
