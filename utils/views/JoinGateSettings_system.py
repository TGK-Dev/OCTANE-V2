...
class JoinGateSettings_Edit(View):
    def __init__(
        self, interaction: Interaction, data: dict, message: discord.Message = None
    ):
    ...
    @button(
        ...
        custom_id="joingate_toggle",
        row=0,
    )
    async def joingate_toggle(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if self.data["joingate"]["enabled"]:
            self.data["joingate"]["enabled"] = False
        else:
            self.data["joingate"]["enabled"] = True
        await interaction.response.edit_message(
            embed=self.update_embed(self.data), view=self
        )

    @button(
        label="Decancer",
        style=discord.ButtonStyle.gray,
        custom_id="joingate_decancer",
        row=0,
    )
    async def joingate_decancer(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if self.data["joingate"]["decancer"]:
            self.data["joingate"]["decancer"] = False
        else:
            self.data["joingate"]["decancer"] = True
        await interaction.response.edit_message(
            embed=self.update_embed(self.data), view=self
        )
    ...
    @button(
        label="Action",
        style=discord.ButtonStyle.gray,
        custom_id="joingate_action",
        row=0,
    )
    async def joingate_action(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        view = View()
        view.value = False
        options = [
            SelectOption(label="Kick", value="kick"),
            SelectOption(label="Ban", value="ban"),
        ]
        view.select = Select_General(
            placeholder="Please select to perform", options=options, max_values=1
        )
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                view=None,
                embed=discord.Embed(
                    description=f"Join Gate action has been updated to {view.select.values[0]}"
                ),
            )
        else:
    ...
    @button(
        label="Account Age",
        style=discord.ButtonStyle.gray,
        custom_id="joingate_accountage",
        row=1,
    )
    async def joingate_accountage(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        view = View()
        view.value = False
        options = [
            SelectOption(label="1 Day", value="1"),
            SelectOption(label="2 Days", value="2"),
            SelectOption(label="3 Days", value="3"),
            SelectOption(label="4 Days", value="4"),
            SelectOption(label="5 Days", value="5"),
            SelectOption(label="6 Days", value="6"),
            SelectOption(label="7 Days", value="7"),
        ]
        view.select = Select_General(
            placeholder="Please select account age", options=options, max_values=1
        )
        view.add_item(view.select)
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                view=None,
                embed=discord.Embed(
                    description=f"Join Gate account age has been updated to {view.select.values[0]} Days"
                ),
            )
        else:
            await interaction.delete_original_response()
    ...
    @button(
        label="Whitelist",
        style=discord.ButtonStyle.gray,
        custom_id="joingate_whitelist",
        row=1,
    )
    async def joingate_whitelist(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        modal = General_Modal(title="Whitelist Users Form", interaction=interaction)
        modal.qestion = TextInput(
            label="Whitelist Users",
            placeholder="Please enter user ids to whitelist",
            max_length=1000,
        )
        if self.data["joingate"]["whitelist"] is not None:
            modal.qestion.default = ",".join(self.data["joingate"]["whitelist"])
    ...
    @button(
        label="Auto Role",
        style=discord.ButtonStyle.gray,
        custom_id="joingate_autorole",
        row=1,
    )
    async def joingate_autorole(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        view = View()
        view.value = False
        view.select = Role_select(
            placeholder="Please select roles to add/remove", max_values=10, min_values=1
        )
        view.add_item(view.select)
        ...
        if view.value:
            ...
            for role in view.select.values:
                if role == interaction.guild.default_role:
                    failed_roles += f"{role.mention}, "
                    continue
                if role.position + 1 > interaction.guild.me.top_role.position:
                    failed_roles += f"{role.mention}, "
                    continue
                if (
    ...
    @button(
        label="Log Channel",
        style=discord.ButtonStyle.gray,
        custom_id="joingate_logchannel",
        row=1,
    )
    async def joingate_logchannel(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        view = View()
        view.value = False
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                view=None,
                embed=discord.Embed(
                    description=f"Join Gate log channel has been updated to {view.select.values[0].mention}"
                ),
            )
        else:
            await interaction.delete_original_response()
    ...
    @button(
        label="Save", style=discord.ButtonStyle.gray, custom_id="joingate_save", row=2
    )
    async def joingate_save(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        self.value = True
