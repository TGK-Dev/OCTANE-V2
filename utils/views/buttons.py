    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id == self.user.id:
            ...
            await interaction.response.send_message(
                "This is not your confirmation menu.", ephemeral=True
            )
            return False
...
class Link_view(View):
    def __init__(self, label, url):
        ...
        self.add_item(
            discord.ui.Button(label=label, url=url, style=discord.ButtonStyle.url)
        )
...
class Reload(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id in [488614633670967307, 301657045248114690]:
            ...
            await interaction.response.send_message(
                "Imagin use this button when you can't even reload me.", ephemeral=True
            )
            return False
    ...
    @discord.ui.button(
        emoji="<:reload:1127218199969144893>",
        style=discord.ButtonStyle.gray,
        custom_id="DEV:RELOAD",
    )
