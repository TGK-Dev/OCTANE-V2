...
class AuctionConfig(View):
    ...
    async def category(self, interaction: Interaction, button: Button):
        embed = discord.Embed(
            description="Select a category for the ticket system to use", color=0x2B2D31
        )
        view = View()
        view.value = None
        ...
        if view.value is None:
            ...
            await interaction.client.auction.update_config(
                interaction.guild.id, self.data
            )
    ...
    async def Request_channel(self, interaction: Interaction, button: Button):
        embed = discord.Embed(
            description="Select a channel for the ticket system to use", color=0x2B2D31
        )
        view = View()
        view.value = None
        view.select = Channel_select(
    ...
    async def manager_roles(self, interaction: Interaction, button: Button):
        ...
        view.select = Role_select(
            placeholder="Please select the roles you want to be able to manage auctions",
            min_values=1,
            max_values=10,
        )
        view.add_item(view.select)
    ...
    async def ping_role(self, interaction: Interaction, button: Button):
        ...
        view.value = None
        view.select = Role_select(
            placeholder="Please select the role you want to be pinged when an auction starts",
            min_values=1,
            max_values=1,
        )
        view.add_item(view.select)
    ...
    @button(
        label="Mini Worth",
        style=discord.ButtonStyle.gray,
        emoji="<:tgk_bank:1073920882130558987>",
        row=2,
    )
    async def mini_worth(self, interaction: Interaction, button: Button):
        ...
        modal.worth = discord.ui.TextInput(
            label="Please enter the minimum worth for an auction", required=True
        )
        modal.worth.default = (
            str(self.data["minimum_worth"]) if self.data["minimum_worth"] else None
        )
    ...
    async def auto_config(self, interaction: Interaction, button: Button):
        await interaction.response.send_message(
            "Please wait while I configure the auction system for you", ephemeral=True
        )
        overwrite = {
        ...
        if any(
            [
                self.data["category"],
                self.data["request_channel"],
                self.data["queue_channel"],
                self.data["bid_channel"],
                self.data["payment_channel"],
                self.data["log_channel"],
            ]
        ):
            view = Confirm(interaction.user, 30)
            await interaction.edit_original_response(
                content="You already have a setup few channels, are you sure you want to continue?",
                view=view,
            )
            view.message = await interaction.original_response()
            await view.wait()
            if view.value is False or None:
                await interaction.response.edit_message(
                    content="Action cancelled", view=None, delete_after=10
                )
                return
            elif view.value:
                await view.interaction.response.edit_message(
                    content="Please wait while I configure the auction system for you",
                    view=None,
                )

        category = await interaction.guild.create_category(
            "Auctions", overwrites=overwrite
        )
        await interaction.guild.create_text_channel(
            name="Rules", category=category, overwrites=overwrite
        )
        self.data["category"] = category.id
        request_channel = await interaction.guild.create_text_channel(
            name="Requests", category=category, overwrites=overwrite
        )
        self.data["request_channel"] = request_channel.id
        queue_channel = await interaction.guild.create_text_channel(
            name="Queue", category=category, overwrites=overwrite
        )
        self.data["queue_channel"] = queue_channel.id
        bid_channel = await interaction.guild.create_text_channel(
            name="Bids", category=category, overwrites=overwrite
        )
        self.data["bid_channel"] = bid_channel.id
        payment_channel = await interaction.guild.create_text_channel(
            name="Payments", category=category, overwrites=overwrite
        )
        self.data["payment_channel"] = payment_channel.id
        logs_channel = await interaction.guild.create_text_channel(
            name="Logs", category=category, overwrites=overwrite
        )
        ...
        embed = await self.update_embed(interaction, self.data)
        await interaction.edit_original_response(
            content="I have finished configuring the auction system for you, you still need to configure the manager roles"
        )
        await interaction.message.edit(embed=embed, view=self)
...
class AuctionReminder(View):
    ...
    @button(
        emoji="<a:tgk_timer:841624339169935390>",
        style=discord.ButtonStyle.gray,
        custom_id="Auction:reminder",
    )
    async def reminder(self, interaction: Interaction, button: Button):
        ...
        if config["hold"]:
            if interaction.user.id in config["reminder"]["reminders"]:
                await interaction.response.send_message(
                    "You already have signed up for reminder", ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                "You will be reminded when the auction starts", ephemeral=True
            )
        else:
            await interaction.response.send_message(
                "Looks like requests are already open", ephemeral=True
            )
