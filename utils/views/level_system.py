...
class LevelingConfig(View):
    def __init__(
        self, user: discord.Member, data: dict, message: discord.Message = None
    ):
        self.user = user
    ...
    async def update_embed(self, interaction: Interaction, leveling_config):
        ...
        for level, role in rewards_roles:
            embed.description += (
                f"> `{level}` : <:tgk_blank:1072224743266193459> <@&{role}>\n"
            )
    ...
    async def interaction_check(self, interaction: Interaction):
        if interaction.user.id == self.user.id:
            ...
            await interaction.response.send_message(
                content="You don't have permission to use this!", ephemeral=True
            )
            return False
    ...
    async def global_multiplier(self, interaction: Interaction, button: Button):
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                content="Global multiplier set!", embed=None, view=None
            )
    ...
    @button(
        ...
        style=discord.ButtonStyle.gray,
        emoji="<:tgk_person_remove:1132022925554372708>",
        row=0,
    )
    async def clear_on_leave(self, interaction: Interaction, button: Button):
        if self.data["clear_on_leave"]:
            ...
            await interaction.response.send_message(
                content="Clear on leave has been disabled!",
                ephemeral=True,
                delete_after=5,
            )
            await self.message.edit(
            ...
            await interaction.response.send_message(
                content="Clear on leave has been enabled!",
                ephemeral=True,
                delete_after=5,
            )
            await self.message.edit(
    ...
    @button(
        ...
        style=discord.ButtonStyle.gray,
        emoji="<:tgk_announce:1123919566427406437>",
        row=1,
    )
    async def announcement_channel(self, interaction: Interaction, button: Button):
        ...
        view.select = Channel_select(
            placeholder="Select the channel you want to set",
            min_values=1,
            max_values=1,
        ...
        if view.value:
            if view.select.values[0].id == self.data["announcement_channel"]:
                ...
                await view.select.interaction.response.edit_message(
                    content="Announcement channel has been disabled!"
                )
            else:
                self.data["announcement_channel"] = view.select.values[0].id
                await view.select.interaction.response.edit_message(
                    content=f"Announcement channel has been set to <#{view.select.values[0].id}>!"
                )
            await view.select.interaction.delete_original_response()
            await self.message.edit(
                embed=await self.update_embed(interaction, self.data)
    ...
    async def multipliers_role(self, interaction: Interaction, button: Button):
        ...
        if multi_view.select.values[0] == "0":
            try:
                ...
                pass
            await multi_view.select.interaction.response.edit_message(
                content=f"Multiplier for {muilt_role.mention} has been removed!",
                embed=None,
                view=None,
            )
        else:
            self.data["multipliers"]["roles"][str(view.select.values[0].id)] = int(
                multi_view.select.values[0]
            )
            await multi_view.select.interaction.response.edit_message(
                content=f"Multiplier for {muilt_role.mention} has been set to {multi_view.select.values[0]}x!",
                embed=None,
                view=None,
            )
    ...
    async def multipliers_channels(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        view.select = Channel_select(
            placeholder="Select the channels you want to set multiplier for",
            min_values=1,
            max_values=1,
            channel_types=[discord.ChannelType.text],
        )
        ...
        if multi_view.select.values[0] == "0":
            try:
                ...
                pass
            await multi_view.select.interaction.response.edit_message(
                content=f"Multiplier for <#{view.select.values[0].id}> has been removed!",
                embed=None,
                view=None,
            )
        else:
            self.data["multipliers"]["channels"][str(view.select.values[0].id)] = int(
                multi_view.select.values[0]
            )
            await multi_view.select.interaction.response.edit_message(
                content=f"Multiplier for <#{view.select.values[0].id}> has been set to {multi_view.select.values[0]}x!",
                embed=None,
                view=None,
            )
    ...
    async def blacklist_channels(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        view.select = Channel_select(
            placeholder="Select the channels you want to blacklist",
            min_values=1,
            max_values=10,
            channel_types=[discord.ChannelType.text],
        )
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                content=f"Blacklisted channels: {add_channels if add_channels else 'None'}\nUnblacklisted channels: {remove_channels if remove_channels else 'None'}",
                embed=None,
                view=None,
            )
            await self.message.edit(
    ...
    async def blacklist_roles(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        view.select = Role_select(
            placeholder="Select the roles you want to blacklist",
            min_values=1,
            max_values=10,
        )
        view.add_item(view.select)
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                content=f"Blacklisted roles: {add_roles if add_roles else 'None'}\nUnblacklisted roles: {remove_roles if remove_roles else 'None'}",
                embed=None,
                view=None,
            )
            await self.message.edit(
    ...
    @button(
        ...
        style=discord.ButtonStyle.gray,
        emoji="<a:nat_message:1063077628036272158>",
        row=3,
    )
    async def weekly_activity(self, interaction: Interaction, button: Button):
        view = General_Modal(
            title="Weekly Activity Requirement", interaction=interaction
        )
        view.value = False
        view.req = TextInput(
            label="Weekly Activity Requirement",
            placeholder="Enter the weekly activity requirement Enter (0) to disable",
            min_length=1,
            max_length=4,
        )
        ...
        if view.req.value == "0":
            ...
            await view.interaction.response.send_message(
                content="Weekly activity has been disabled!", embed=None, view=None
            )
        else:
            try:
                ...
                return await view.interaction.response.send_message(
                    content="Invalid value entered!", embed=None, view=None
                )
            await view.interaction.response.send_message(
                content=f"Weekly activity requirement has been set to {view.req.value} messages!\nNote: You can't this info in config embed",
                embed=None,
                view=None,
                ephemeral=True,
            )
    ...
    async def weekly_role(self, interaction: Interaction, button: Button):
        view = View()
        view.select = Role_select(
            placeholder="Select the role you want to reward", min_values=1, max_values=1
        )
        view.add_item(view.select)
        view.value = False
        await interaction.response.send_message(view=view, ephemeral=True)
        ...
        if not view.value:
            return
        self.data["weekly"]["role"] = view.select.values[0].id
        await interaction.client.level.update_config(interaction.guild, self.data)
        await self.message.edit(embed=await self.update_embed(interaction, self.data))
    ...
    async def reward_roles(self, interaction: Interaction, button: Button):
        ...
        view.value = False
        view.select = Role_select(
            placeholder="Select the roles you want to reward",
            min_values=1,
            max_values=1,
        )
        view.add_item(view.select)
        ...
        modal.level = TextInput(
            label="Reward Level",
            placeholder="Enter the level you want to reward",
            min_length=1,
            max_length=3,
        )
        ...
        await modal.interaction.response.edit_message(
            content=f"{role.mention} will now be rewarded at level {level}!",
            embed=None,
            view=None,
        )
