...
class LevelingConfig(View):
    ...
    async def weekly_activity(self, interaction: Interaction, button: Button):
        view = General_Modal(title="Weekly Activity Requirement", interaction=interaction)
        view.value = False
    ...
    async def weekly_role(self, interaction: Interaction, button: Button):
        view = View()
        view.select = Role_select(placeholder="Select the role you want to reward",min_values=1, max_values=1)
        view.add_item(view.select)
        view.value = False
        await interaction.response.send_message(view=view, ephemeral=True)
        await view.wait()
