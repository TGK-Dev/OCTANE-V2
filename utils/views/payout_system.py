...
class Payout_Config_Edit(discord.ui.View):
    ...
    async def interaction_check(self, interaction: Interaction):
        if interaction.user.id == self.user.id:
            ...
            await interaction.response.send_message(
                "you can't use this view", ephemeral=True
            )
            return False
    ...
    @discord.ui.button(
        ...
        emoji="<:channel:1017378607863181322>",
        row=0,
    )
    async def queue_channel(
        ...
        await interaction.response.send_message(
            content="Select a new channel from the dropdown menu below",
            view=view,
            ephemeral=True,
        )
        await view.wait()
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                content="Suscessfully updated queue channel", view=None
            )
            ...
            await interaction.edit_original_response(
                content="No channel selected", view=None
            )
    ...
    async def claim_channel(
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                content="Suscessfully updated claim channel", view=None
            )
            embed = self.update_embed(self.data, interaction)
            await interaction.client.payout_config.update(self.data)
            await interaction.message.edit(embed=embed)
    ...
    async def payout_channel(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        view = discord.ui.View()
        view.value = False
        view.select = Channel_select(
            "Select new payout channel",
            max_values=1,
            min_values=1,
            disabled=False,
            channel_types=[discord.ChannelType.text],
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                content="Suscessfully updated payout channel", view=None
            )
            embed = self.update_embed(self.data, interaction)
            await interaction.client.payout_config.update(self.data)
            await interaction.message.edit(embed=embed)
    ...
    async def log_channel(
        ...
        if view.value:
            ...
            await view.select.interaction.response.edit_message(
                content="Suscessfully updated log channel", view=None
            )
            embed = self.update_embed(self.data, interaction)
            await interaction.message.edit(embed=embed)
            await interaction.client.payout_config.update(self.data)
    ...
    @discord.ui.button(
        label="Manager Role",
        style=discord.ButtonStyle.gray,
        emoji="<:role_mention:1063755251632582656>",
        row=1,
    )
    ...
    @discord.ui.button(
        label="Claim Time",
        style=discord.ButtonStyle.gray,
        emoji="<:octane_claim_time:1071517327813775470>",
        row=2,
    )
    async def claim_time(
        ...
        modal.question = discord.ui.TextInput(
            ...
            placeholder="Enter New Claim Time exp: 1h45m",
            min_length=1,
            max_length=10,
        )
        modal.value = None
        ...
        if modal.value:
            ...
            if time < 3600:
                await modal.interaction.response.send_message(
                    "Claim time must be at least 1 hour", ephemeral=True
                )
...
class Payout_claim(discord.ui.View):
    ...
    async def payout_claim(
        ...
        current_embed.description = current_embed.description.replace(
            "`Pending`", "`Claimed`"
        )
        ...
        await interaction.edit_original_response(
            embed=discord.Embed(
                description="<:octane_yes:1019957051721535618> | Sucessfully claimed payout, you will be paid in 24hrs",
        ...
        if payout_data["set_by"] != interaction.user.id:
            ...
            if modal.value:
                ...
                await interaction.message.edit(
                    embed=embed, view=temp_view, content=None
                )
                await interaction.client.payout_queue.delete(interaction.message.id)
...
class Payout_Buttton(discord.ui.View):
    ...
    async def get_command(self, winner: discord.Member, amount: str, item=None):
        if item is None:
            ...
            return (
                f"/serverevents payout user:{winner.id} quantity:{amount} item:{item}"
            )
    ...
    async def reject(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            "Are you sure you want to reject this payout?", view=view, ephemeral=False
        )
        await view.wait()
        if not view.value:
        ...
        if not data:
            return await view.interaction.response.edit_message(
                embed=discord.Embed(
                    description="<:dynoError:1000351802702692442> | Payout not found in Database",
                    color=discord.Color.red(),
                )
            )
        ...
        embed = interaction.message.embeds[0]
        embed.description = embed.description.replace(
            "`Awaiting Payment`", "`Payout Rejected`"
        )
        ...
        await view.interaction.response.edit_message(
            embed=discord.Embed(
                description="<:octane_yes:1019957051721535618> | Payout Rejected Successfully!",
                color=interaction.client.default_color,
            ),
            view=None,
        )
        ...
        try:
            ...
            if "⏣" in items:
                ...
                if items == data["prize"]:
                    ...
                    return await view.interaction.edit_original_response(
                        content="The prize of the provided message is not the prize of this payout"
                    )
            else:
            ...
            payot_embed.description = payot_embed.description.replace(
