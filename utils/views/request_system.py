...
class Config(View):
    ...
    async def interaction_check(self, interaction: Interaction):
        ...
        await interaction.response.send_message(
            "Only the person who opened this config can use it", ephemeral=True
        )
        return False
    ...
    async def _manager_roles(
        ...
        view.select = Role_select(
            placeholder="Select roles your want too add/remove",
            min_values=1,
            max_values=10,
        )
    ...
    async def _channels(
        ...
        view.select = Select_General(
            placeholder="Select which channel settings you want to edit",
            max_values=1,
            options=[
                SelectOption(
                    label="Request Channel",
                    value="request_channel",
                    description="The channel where members can request events",
                    emoji="<:tgk_bid:1114854528018284595>",
                ),
                SelectOption(
                    label="Request Queue",
                    value="request_queue",
                    description="The channel where verified requests are sent",
        ...
        channel_view.select = Channel_select(
            placeholder="Select a channel which you want to set",
            max_values=1,
            min_values=1,
        ...
        await channel_view.select.interaction.response.edit_message(
            content=f"Set {operation} to {channel.mention}", view=None
        )
        await self.update_embed(interaction, self.data)
        await interaction.client.events.update_config(self.data)
    ...
    async def _events(
        ...
        view.value = None
        view.select = Select_General(
            placeholder="Select which event settings you want to edit",
            max_values=1,
            options=[
                SelectOption(
                    label="Add Event",
        ...
        if operation == "add_event":
            ...
            modal.req_dono = TextInput(
                label="Minimum Dono",
                placeholder="Minimum Dono Required",
                min_length=1,
                max_length=100,
                style=TextStyle.short,
                required=True,
            ...
            req_dono = await DMCConverter_Ctx().convert(
                modal.interaction, modal.req_dono.value
            )
            embed = discord.Embed(
                description="Event Name: {}\nRequired Dono: {}".format(name, req_dono),
                color=interaction.client.default_color,
            )
            ...
            await self.update_embed(interaction, self.data)
            await confrim.interaction.response.edit_message(
                content="Event Created", view=None
            )
...
class Event_view(View):
    ...
    async def on_interaction(self, interaction: Interaction):
        ...
        if not guild_config:
            await interaction.response.send_message(
                "Event system is not setup", ephemeral=True
            )
            return
    ...
    async def _hosted(self, interaction: Interaction, button: discord.ui.Button):
        ...
        modal_link = TextInput(
            label="Event Link",
            placeholder="Event Thread Link",
            max_length=1000,
        ...
        await interaction.client.events.delete(
            {"_id": interaction.message.id, "guild_id": interaction.guild.id}
        )
