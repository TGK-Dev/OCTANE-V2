...
class Auction_db:
        ...
        if not config:
            ...
            if not config:
                config = {
                    ...
                    "minimum_worth": None,
                    "hold": False,
                    "reminder": {"message_id": None, "reminders": []},
                }
    ...
    async def verfiy_payment(
        self, user: discord.Member, thread: discord.Thread, item: str, quantity: int
    ):
        def check(m: discord.Message):
            ...
            if len(m.embeds) == 0:
                return False
            if m.embeds[0].description == "Successfully donated!":
                return True
...
class Auction(commands.GroupCog):
    ...
    async def update_message(self, message: discord.Message, data: dict, first: bool):
        if first:
            timestamp30s = int(
                (datetime.datetime.now() + datetime.timedelta(seconds=30)).timestamp()
            )
            embed = message.embeds[0]
            fields = [field.name for field in embed.fields]
            if "Time Left" in fields:
                embed.set_field_at(
                    fields.index("Time Left"),
                    name="Time Left",
                    value=f"<t:{timestamp30s}:R>",
                )
            await message.edit(embed=embed)
        else:
            timestamp10s = int(
                (datetime.datetime.now() + datetime.timedelta(seconds=10)).timestamp()
            )
            ...
            if "Time Left" in fields:
                embed.set_field_at(
                    fields.index("Time Left"),
                    name="Time Left",
                    value=f"<t:{timestamp10s}:R>",
                )
            if "Current Bidder" in fields:
                embed.set_field_at(
                    fields.index("Current Bidder"),
                    name="Current Bidder",
                    value=f"<@{data['current_bidder']}>",
                )
            if "Current Bid" in fields:
                embed.set_field_at(
                    fields.index("Current Bid"),
                    name="Current Bid",
                    value=f"{data['current_bid']:,}",
                )
            await message.edit(embed=embed)
    ...
    async def auction_loop(self):
        ...
        for key, value in auctions.items():
            ...
            if auction["call_started"]:
                continue
            call_time = auction["last_bet"] + datetime.timedelta(
                seconds=auction["time_left"]
            )
            if call_time < datetime.datetime.now():
                auction["call_started"] = True
                await auction["thread"].send(
                    f"No Bet was place in last {auction['time_left']} seconds, starting closing calls"
                )
    ...
    async def request(
        ...
        if not config["request_channel"]:
            return await interaction.response.send_message(
                "Request channel is not setup yet!"
            )

        queue_data = await self.backend.auction.find_many_by_custom(
            {"user_id": interaction.user.id, "ended": False}
        )
        if len(queue_data) >= 2:
            return await interaction.response.send_message(
                "You already have 2 auction request in queue!"
            )
        ...
        if item["price"] * quantity < config["minimum_worth"]:
            return await interaction.response.send_message(
                f"Minimum price of auction is ⏣ {config['minimum_worth']:,}!"
            )
        embed = discord.Embed(
        ...
        thread = await msg.create_thread(
            name=f"Auction Verification - {interaction.user.name}",
        ...
        try:
            ...
            pass
        await thread.send(
            f"{interaction.user.mention}, Please donate the auction items in this thread to verify your request."
        )
        await thread.send(
            f"`/serverevents donate quantity:{quantity} item: {item['_id']}`"
        )
        verified = await self.backend.verfiy_payment(
            interaction.user, thread, item["_id"], quantity
        )

        if not verified:
            await thread.send(
                f"{interaction.user.mention}, Your request has been Failed to verify!"
            )
            await thread.edit(
                name=f"Auction Request - {interaction.user.name} - Failed",
                locked=True,
                archived=True,
            )
            embed.title += " - Failed"
            ...
            return
        await thread.send(
            f"{interaction.user.mention}, Your request has been verified!"
        )
        data = {
        ...
        await thread.edit(
            name=f"Auction Request - {interaction.user.name} - Verified",
        ...
        queue_embed.set_author(
            name=f"{interaction.user.name}'s Auction",
    ...
    async def start(self, interaction: discord.Interaction):
        ...
        if not config["bid_channel"]:
            return await interaction.response.send_message(
                "Bid channel is not setup yet!"
            )
        if interaction.channel.id != config["bid_channel"]:
            return await interaction.response.send_message(
                "You can only use this command in bid channel!", ephemeral=True
            )

        auction_data = await self.backend.auction.find_by_custom(
            {"guild_id": interaction.guild_id, "ended": False}
        )
        if not auction_data:
            return await interaction.response.send_message(
                "There are no auctions pending!", ephemeral=True
            )
        item = self.bot.dank_items_cache.get(auction_data["item"])

        if item["_id"] in config["banned_items"]:
            return await interaction.response.send_message(
                "This item is banned from auction!", ephemeral=True
            )
        ...
        if seller is None:
            ...
            return await interaction.response.send_message(
                "Seller is not in the server anymore, auction has been cancelled! Please start again!",
                ephemeral=True,
            )
        ...
        embed.set_author(
            ...
            icon_url="https://cdn.discordapp.com/emojis/1134834084728815677.webp?size=96&quality=lossless",
        )
        embed.add_field(
            name="Seller",
            value=interaction.guild.get_member(auction_data["user_id"]).mention,
        )
        embed.add_field(
            name="Item", value=f"`{auction_data['quantity']}x` **{item['_id']}**"
        )
        ...
        thread = await msg.create_thread(
            name=f"Auction for {item['_id']}", auto_archive_duration=1440
        )
        data = {
            "_id": thread.id,
            "item": item["_id"],
    ...
    async def re_queue(self, interaction: discord.Interaction, message: str):
        ...
        if not data["ended"]:
            return await interaction.response.send_message(
                "Auction is not ended yet!", ephemeral=True
            )
        data["ended"] = False
    ...
    async def item_ban(self, interaction: discord.Interaction, item: str):
        ...
        if item in config["banned_items"]:
            ...
            await interaction.response.send_message(
                f"**{item}** has been unbanned from auction!"
            )
        else:
            config["banned_items"].append(item)
            await interaction.response.send_message(
                f"**{item}** has been banned from auction!"
            )
        await self.backend.update_config(interaction.guild_id, config)

    @app_commands.command(
        name="start_from_pool", description="Start an auction from pool"
    )
    @app_commands.autocomplete(item=item_autocomplete)
    @app_commands.describe(
        item="Item to start auction for",
        quantity="Quantity of item to start auction for",
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def start_from_pool(
        self, interaction: discord.Interaction, item: str, quantity: int = 1
    ):
        config = await self.backend.get_config(interaction.guild_id)
        item = self.bot.dank_items_cache.get(item)
    ...
    async def queue(self, interaction: discord.Interaction):
        ...
        data = await self.backend.auction.find_many_by_custom(
            {"ended": False, "guild_id": interaction.guild_id}
        )
        pages = []
        if len(data) == 0:
            return await interaction.response.send_message(
                "No auction is running right now!", ephemeral=True
            )
        for auction in data:
            ...
            embed = discord.Embed(
                title=f"Auction for {auction['item']}",
                description="",
                color=interaction.client.default_color,
            )
            embed.set_author(
            ...
            embed.add_field(
                name="Item", value=f"`{auction['quantity']}x` **{auction['item']}**"
            )
            embed.add_field(
                name="Worth", value=f"⏣ {int(item['price'] * auction['quantity']):,}"
            )
            pages.append(embed)
    ...
    async def close(self, interaction: discord.Interaction):
        ...
        if config["hold"]:
            return await interaction.response.send_message(
                "Auction request channel is already closed!"
            )
        ...
        await req_channel.set_permissions(
    ...
    async def open(self, interaction: discord.Interaction):
        ...
        if config["hold"]:
            return await interaction.response.send_message(
                "Auction request channel is already open!"
            )
        ...
        for i in range(0, len(config["reminder"]["reminders"]), 15):
            members = [
                "<@" + str(member) + ">"
                for member in config["reminder"]["reminders"][i : i + 15]
            ]
            await req_channel.send(",".join(members), delete_after=2)
        await req_channel.send(
            "Hey! Request channel is now open, you can request an auction now!",
        ...
        if message.author.bot:
            ...
            if message.channel.id in self.backend.payment_cache.keys():
                if len(message.embeds) == 0:
                    return
                if message.embeds[0].description is None or "":
                    return
        ...
        if ammout > data["current_bid"]:
            if ammout - data["current_bid"] < data["bet_increment"]:
                return await message.reply(
                    f"Your bet must have at least ⏣ {data['bet_increment']:,} increment from the current bid!"
                )
            ...
            return await message.reply(
                f"Current bid is ⏣ {data['current_bid']:,}! You can't bet less than that!"
            )
    ...
    async def on_auction_calls(self, data: dict, call_num: int):
        ...
        if call_num == 0:
            embed = discord.Embed(
                description="# Going Once...", color=self.bot.default_color
            )
        if call_num == 1:
            embed = discord.Embed(
                description="# Going Twice...", color=self.bot.default_color
            )
        if call_num >= 2:
        ...
        def check(m: discord.Message):
            ...
            if m.channel != thread:
                return False
            if m.author.id == data["host"]:
                return False
            if m.author.id == data["current_bidder"]:
                return False
        ...
        while True:
            try:
                ...
                if not isinstance(ammout, int):
                    time_passed = int(
                        (datetime.datetime.now() - call_started).total_seconds()
                    )
                    continue
                if not ammout:
                    time_passed = int(
                ...
                if ammout > data["current_bid"]:
                    if ammout - data["current_bid"] < data["bet_increment"]:
                        await msg.reply(
                            f"You can't bet less than ⏣ {data['bet_increment']:,} more than the current bid!"
                        )
                        time_passed = int(
                            (datetime.datetime.now() - call_started).total_seconds()
                        )
                        ...
                        await msg.reply(
                            f"You are now the highest bidder with ⏣ {ammout:,}!"
                        )
    ...
    async def on_auction_ended(self, data: dict):
        ...
        if (
            ...
            and data["current_bid"] == data["starting_bid"]
        ):
            temp_data = await self.backend.auction.find(data["db_id"])
            if temp_data is None:
                await message.reply(
                    "No one bid on your auction, so it has been cancelled"
                )
                ...
                await message.reply(
                    "No one bid on your auction, so it has been cancelled and put back in queue!"
                )
                await thread.send(
                    "No one bid on this auction, so it has been cancelled and put back in queue!"
                )
                try:
        ...
        third_call = discord.Embed(
            description="# Going Thrice...", color=self.bot.default_color
        )
        embed = discord.Embed(
            description=f"# Sold to <@{data['current_bidder']}> for ⏣ {data['current_bid']:,}!",
            color=0x00FF00,
        )
        ...
        embed.description += (
            f"**Item:** `{payout_data['quantity']}x` {payout_data['item']}\n"
        )
        embed.description += f"**Winning Bid:** ⏣ {payout_data['bid']:,}\n"

        msg: discord.Message = await payout_channel.send(
            embed=embed,
            content=f"<@{payout_data['bidder']}>, Please pay {payout_data['bid']:,} in the thread below to confirm your purchase!",
        )
        payout_data["_id"] = msg.id
        thread = await msg.create_thread(
            name="Auction Payment", auto_archive_duration=1440
        )
    ...
    async def on_payment(self, message: discord.Message):
        if message.embeds[0].description == "Successfully donated!":
            ...
            if not message.reference:
                return
            try:
                donate_mesage = await message.channel.fetch_message(
            ...
            if quantity != data["bid"]:
                return await message.channel.send(
                    f"<@{data['bidder']}>, You have to pay ⏣ {data['bid']:,}!"
                )
            await message.channel.send(
                f"<@{data['bidder']}>, Thank you for your payment! your items will be delivered shortly!"
            )
            data["paid"] = True
            await self.backend.payment.update(data)
            await message.channel.send(
                f"<@{data['host']}>, We have received the payment for your auction! your payment will be credited shortly!"
            )
            await message.channel.send(
                "Dank Manager, Here is your commands for this auction:"
            )
            await message.channel.send(
                f"/serverevents payout user:{data['host']} quantity:{data['bid']}"
            )
            await message.channel.send(
                f"/serverevents payout user:{data['bidder']} quantity:{data['quantity']} item:{data['item']}"
            )

    @commands.Cog.listener()
    async def on_auction_start_log(
        self,
        user: int,
        host: int,
        ...
        embed = discord.Embed(
            color=self.bot.default_color, description="", title="Auction | Started"
        )
        embed.set_author(
            name="Auction Manager",
            icon_url="https://cdn.discordapp.com/emojis/1134834084728815677.webp?size=96&quality=lossless",
    ...
    async def on_auction_end_log(
        ...
        embed = discord.Embed(
            color=self.bot.default_color, description="", title="Auction | Ended"
        )
        embed.set_author(
            name="Auction Manager",
            icon_url="https://cdn.discordapp.com/emojis/1134834084728815677.webp?size=96&quality=lossless",
