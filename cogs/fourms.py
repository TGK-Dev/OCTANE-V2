...
class Forums(commands.Cog):
    ...
    async def _close(self, ctx: commands.Context):
        ...
        if not isinstance(ctx.channel, discord.Thread): 
            return await ctx.send("This command can only be used in threads.")
        if not isinstance(ctx.channel.parent, discord.ForumChannel): 
            return await ctx.send("This command can only be used in forums.")
        if ctx.channel.parent_id != 1246360445292515390: 
            return
        tag = ctx.channel.parent.get_tag(1247099210873180262)
        if tag is None:
            return await ctx.send("This forum does not have a tag.")
