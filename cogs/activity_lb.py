...
class ActivityLeaderboard(commands.GroupCog, name="activity"):
    ...
    async def update_leaderboard(self):
        ...
        embed = discord.Embed(
            ...
            description="",
            timestamp=datetime.datetime.now().astimezone(pytz.timezone("Asia/Kolkata")),
        )
        for index, user in enumerate(users[:10]):
            ...
            if not member:
                continue
            embed.description += (
                f"{index + 1}. {member.mention} - {user['messages']} messages\n"
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if self.config_cach == {}:
            return
        if message.channel.id != self.config_cach["channel"]:
            return
        ...
        if (
            user_data["last_message"] + datetime.timedelta(seconds=10)
    ...
    @app_commands.command(
        name="set-channel",
        description="Set the channel for the message tracking leaderboard",
    )
    async def set_channel(self, interaction: Interaction, channel: discord.TextChannel):
        ...
        await interaction.response.send_message(
            f"Set the channel to {channel.mention}", ephemeral=True
        )
    ...
    async def set_lb(self, interaction: Interaction, channel: discord.TextChannel):
        ...
        if not config:
            await interaction.response.send_message(
                "You need to set the message tracking channel first", ephemeral=True
            )
            return
        ...
        self.config_cach = config
        await interaction.response.send_message(
            f"Set the leaderboard channel to {channel.mention}", ephemeral=True
        )

    @app_commands.command(
        name="reset", description="Reset the message count for a user"
    )
    async def reset(self, interaction: Interaction, member: discord.Member):
        user_data = await self.messages.find(member.id)
        if not user_data:
            await interaction.response.send_message(
                "This user has no messages", ephemeral=True
            )
            return
        user_data["messages"] = 0
        await self.messages.update(user_data)
        await interaction.response.send_message(
            f"Reset the message count for {member.mention}", ephemeral=True
        )
    ...
    async def reset_lb(self, interaction: Interaction):
        users = await self.messages.get_all()
        await interaction.response.send_message(
            "Resetting the leaderboard", ephemeral=True
        )
        data = []
        for user in users:
            user["messages"] = 0
        ...
        await interaction.edit_original_response(
            content="Leaderboard reset successfully!"
        )


async def setup(bot):
    await bot.add_cog(
        ActivityLeaderboard(bot), guilds=[discord.Object(785839283847954433)]
    )
