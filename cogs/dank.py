...
class Dank_Events(commands.GroupCog, name="dank"):
    ...
    async def item_stats(self, interaction: discord.Interaction, item: str):
        ...
        embed = discord.Embed(
            title=item["_id"], color=interaction.client.default_color, description=""
        )
        ...
        embed.description += "**Price History:**\n"
        price_history = "```diff\n"
    ...
    @app_commands.describe(
        item="The item to update the price of", price="The new price of the item"
    )
    async def item_update(
        self, interaction: discord.Interaction, item: str, price: int
    ):
        item = await self.bot.dank_items.find(item)
        if not item:
            item = {
            ...
            embed = discord.Embed(
                description=f"Added `{item['_id']}` with price `⏣ {price}`",
                color=interaction.client.default_color,
            )
        else:
            old_price = item["price"]
            update_data = {
                "day": item["last_updated"].strftime("%d/%m/%Y"),
                "old_price": old_price,
                "new_price": price,
            }
            ...
            self.bot.dank_items_cache[item["_id"]] = item
        embed = discord.Embed(
            description=f"Updated `{item['_id']}` from `⏣ {old_price}` to `⏣ {price}`",
            color=interaction.client.default_color,
        )
        await interaction.response.send_message(embed=embed)
    ...
    async def item_delete(self, interaction: discord.Interaction, item: str):
        ...
        await interaction.response.send_message(
            "Item deleted successfully", ephemeral=True
        )
    ...
    async def item_force_scrape(self, interaction: discord.Interaction, message: str):
        ...
        await interaction.edit_original_response(
            embed=discord.Embed(
                description="Message Found starting scrape...",
                color=interaction.client.default_color,
            ),
            content=None,
        )
        ...
        for item in items:
            ...
            if not data:
                ...
                update_data = {
                    "day": data["last_updated"].strftime("%d/%m/%Y"),
                    "old_price": old_price,
                    "new_price": price,
                }
...
async def setup(bot):
    await bot.add_cog(
        ...
        guilds=[
            ...
            discord.Object(947525009247707157),
        ],
    )
