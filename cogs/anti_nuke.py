        ...
        if view.value:
            ...
            for channel in view.select.values:
                if channel.id in config['lockdown']['channels']:
                    ...
                    remove_channels += f"{channel.mention}"
                else:
                    config['lockdown']['channels'].append(channel.id)
                    add_channels += f"{channel.mention}"
            embed = discord.Embed(color=interaction.client.default_color, description="")
            embed.description += f"Added Channels: {','.join([channel for channel in add_channels]) if len(add_channels) > 0 else '`None`'}\n"
            embed.description += f"Removed Channels: {','.join([channel for channel in remove_channels]) if len(remove_channels) > 0 else '`None`'}"
