...
class Linked_Roles(commands.GroupCog, name="linkedroles"):
    ...
    async def get_metadata(self, token: str):
        url = (
            f"{self.base_url}/users/@me/applications/{self.bot.user.id}/role-connection"
        )
    ...
    async def update_metadata(self, token: str, meta_data: dict):
        ...
        response = await self.session.put(
            url, headers=headers, data=json.dumps(meta_data)
        )
    ...
    async def refresh_loop(self):
        ...
        for user in data:
            ...
            user["expires_at"] = discord.utils.utcnow() + datetime.timedelta(
    ...
    @app_commands.command(
        name="link", description="Link your discord account to with OCTANE"
    )
    async def _link(self, interaction: Interaction):
        embed = discord.Embed(
            description="Press the button below to link your discord account with OCTANE",
        ...
        view.add_item(
            discord.ui.Button(
                ...
                url="https://tgk-api.vercel.app/api/linked-role/auth",
                emoji="🔗",
            )
        )
    ...
    @app_commands.describe(
        user="The user you want to update the roles for",
        link="The link you want to update",
        value="The value you want to set the link to",
    )
    ...
    async def _update(
        ...
        if data is None:
            data = {
                ...
                "scope": None,
                "metadata": {
                    "platform_name": "The Gambler's Kingdom",
                    "platform_username": user.name,
                    "metadata": {},
                },
            }
            data["metadata"]["metadata"][link.value] = 1 if value else 0
            await self.bot.auth.update(data)
            embed = discord.Embed(
                description=f"User {user.mention} is not linked with OCTANE but metadata has been created for them",
        ...
        if data["access_token"] is None:
            embed = discord.Embed(
                description=f"User {user.mention} is not linked with OCTANE",
                color=discord.Color.red(),
            )
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        if response == "Error":
            embed = discord.Embed(
                description="Faild Due to some error", color=discord.Color.red()
            )
            return await interaction.response.send_message(embed=embed, ephemeral=True)

        embed = discord.Embed(
            description=f"Successfully updated {user.mention}'s linked roles {link.name} to {value}",
    ...
    async def _show(self, interaction: Interaction, user: discord.Member = None):
        ...
        if data is None:
            return await interaction.response.send_message(
                f"{'you' if user.id == interaction.user.id else user.mention} have not linked your account yet",
                ephemeral=True,
            )

        embed = discord.Embed(
            title=f"{user.name}'s linked roles",
            color=discord.Color.blurple(),
            description="",
        )
        embed.description += f"**Platform Name:** {data['metadata']['platform_name']}\n"
        embed.description += (
            f"**Platform Username:** {data['metadata']['platform_username']}\n"
        )
