...
class Event(commands.GroupCog, name="event"):
    ...
    async def _request(
        ...
        if not config["request_channel"]:
            await interaction.response.send_message(
                "No request channel set", ephemeral=True
            )
            return
        if not config["request_queue"]:
            await interaction.response.send_message(
                "No request queue set", ephemeral=True
            )
            return
        if interaction.channel.id != config["request_channel"]:
            await interaction.response.send_message(
                f"You can only use this command in the request channel <#{config['request_channel']}>",
                ephemeral=True,
            )
            return
        ...
        if item:
            ...
            if amount < config["events"][event]["min_amount"]:
                await interaction.response.send_message(
                    f"Minimum amount is {config['events'][event]['min_amount']}",
                    ephemeral=True,
                )
                return
        ...
        thread: discord.Thread = await message.create_thread(
            name="Donation Verification", auto_archive_duration=1440
        )
        ...
        donated = await self.backend.verfiy_donation(
            interaction.user, thread, amount, item if item else None
        )
        if not donated:
            await thread.send(
                f"{interaction.user.mention} did not provide the correct donation/have not donated within 5 minutes"
            )
            await thread.edit(locked=True, archived=True)
            return

        await thread.send(
            f"{interaction.user.mention} your event request has been received and will be processed shortly"
        )
        ...
        queue_embed.description += "**Status:** Pending\n"
        ...
        view.add_item(
            discord.ui.Button(
                ...
                url=f"https://canary.discord.com/channels/{interaction.guild.id}/{thread.id}",
    ...
    async def _start(self, interaction: Interaction, event: str):
        ...
        if len(oldest_pending_event) == 0:
            return await interaction.response.send_message(
                "No pending events", ephemeral=True
            )
        pages = []
