...
class Dev(commands.Cog, name="dev", description="Dev commands"):
    ...
    @app_commands.choices(
        servers=[
            app_commands.Choice(name="NAT", value="e3ea1246"),
            app_commands.Choice(name="OCT∆NΞ", value="dd089fbe"),
            app_commands.Choice(name="A.C.E", value="f9a3bf56"),
        ]
    )
    async def _host(
        self,
        ...
        servers: str,
        signal: Literal["kill", "restart", "start"],
    ):
        view = Confirm(interaction.user, 30)
        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"Are you sure you want send {signal} signal to {servers}",
                color=interaction.client.default_color,
            ),
            view=view,
        )
        ...
        if view.value:
            await view.interaction.response.edit_message(
                embed=discord.Embed(
                    description=f"Successfully sent {signal} signal to {servers}",
                    color=interaction.client.default_color,
                ),
                view=None,
            )
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        f"https://control.sparkedhost.us/api/client/servers/{servers}/power",
                        headers=headders,
    ...
    async def _eval(self, ctx: commands.Context, *, code):
        if ctx.author.id not in self.bot.owner_ids:
            await ctx.reply(
                f"{ctx.author.mention} Wow, you're about as qualified to use that command as I am to perform open-heart surgery with a spork. Maybe stick to coloring books instead?",
                mention_author=True,
            )
            return
...
class Admin(commands.GroupCog):
    ...
    @app_commands.describe(
        user="The user to blacklist", reason="The reason for blacklisting the user"
    )
    @app_commands.checks.has_any_role(
        785842380565774368, 785845265118265376, 787259553225637889
    )
    async def add(
        ...
        if user.id in [self.bot.user.id, 761834680395497484, 301657045248114690]:
            return await interaction.response.send_message(
                embed=discord.Embed(
                    description="You can't blacklist me or my developers",
                    color=interaction.client.default_color,
                ),
                ephemeral=True,
            )
        ...
        if data:
            return await interaction.response.send_message(
                embed=discord.Embed(
                    description=f"{user.mention} is already blacklisted for {data['reason']}",
                    color=interaction.client.default_color,
                ),
                ephemeral=True,
            )
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"{user.mention} has been blacklisted for {reason}",
                color=interaction.client.default_color,
            ),
            ephemeral=False,
        )
        ...
        embed = discord.Embed(
            title="Blacklisted | Added", color=discord.Color.red(), description=""
        )
    ...
    @app_commands.describe(
        ...
        reason="The reason for removing the user from the blacklist",
    )
    ...
    async def remove(
        ...
        embed = discord.Embed(
            title="Blacklisted | Removed", color=discord.Color.green(), description=""
        )
        embed.description += f"**User:** {user.mention}\n"
        embed.description += f"**ID:** {user.id}\n"
    ...
    async def list(self, interaction: discord.Interaction):
        ...
        for i in data:
            ...
            embed.description += (
                f"**Banned At:** {i['banne_at'].strftime('%d/%m/%Y %H:%M:%S')}\n"
            )
            page.append(embed)
    ...
    @commands.group(
        name="admin", description="Admin commands", invoke_without_command=False
    )
    ...
    @admin.command(
        name="snipe",
        description="resets the channels snipes and editsnipes",
        aliases=["rs", "rsnipe"],
    )
    @commands.has_any_role(785845265118265376, 785842380565774368, 941772431750750218)
    async def reset_snipe(self, ctx, channel: discord.TextChannel = None):
        if not channel:
            channel = ctx.channel
        if (
            channel.id not in self.bot.snipes.keys()
            and channel.id not in self.bot.esnipes.keys()
        ):
            return await ctx.send(
                embed=discord.Embed(
                    description=f"There are no snipes in {channel.mention}",
                    color=self.bot.default_color,
                )
            )
        if channel.id in self.bot.snipes.keys():
            del self.bot.snipes[channel.id]
        if channel.id in self.bot.esnipes.keys():
            del self.bot.esnipes[channel.id]
        await ctx.send(
            embed=discord.Embed(
                description=f"Snipes in {channel.mention} have been reset",
                color=self.bot.default_color,
            )
        )
